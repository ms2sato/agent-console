...
A related protocol-hygiene constraint uses the same mechanical-enforcement pattern: the embedded-agent subprocess loop's stdout is the NDJSON wire-protocol channel to the server, so `console.log`/`console.info`/`console.debug`/`process.stdout.write`/`Bun.stdout` are forbidden anywhere in `packages/embedded-agent/src/**` except the wire-protocol writer itself. `scripts/check-embedded-agent-stdout-writers.mjs` (CI: `embedded-agent-stdout-lint`) AST-parses every source file and fails on a non-allowlisted hit. Run it locally via `bun run check:embedded-agent-stdout`.

**Static import order determines which mock a dependency binds to.** `bun test` runs every selected file in ONE process, and `mock.module()`'s registration is permanent for that process's lifetime, so the mocks a test file's own static imports install become every later-loaded file's mocks too. A test file must import a central mock helper (`mock-fs-helper.ts`, `mock-open-helper.ts`, etc.) BEFORE any of its other static imports whose graph reaches production code — a static import placed after the mock helper in the same file still binds correctly (module evaluation follows declaration order), but a static import placed BEFORE it does not, because the module it reaches gets evaluated, and its own bindings resolved, against whatever mock state existed at that earlier point (real `node:fs`, if no memfs-using file has run yet in this process; a differently-shaped mock, if one has). This is exactly how `packages/server/src/routes/__tests__/workers.test.ts` masked a missing-`default` mock factory for as long as it did: the file's own import of `../routes/api.js` (reaching `open` -> `is-wsl`'s `import fs from 'node:fs'`) came before its import of `mock-fs-helper.js`, so the file passed whenever it ran first or alone -- the real `node:fs` bound before the mock ever existed -- and only failed once some other file's mock-fs-helper import had already run earlier in the same process.
...
A test belongs in `packages/server`'s real-fs second `bun test` invocation (`package.json`, Issue [#1699](https://github.com/ms2sato/agent-console/issues/1699)) if and only if it asserts something memfs cannot simulate — mode bits, gid / setgid inheritance, symlink + `mkdir` semantics, `/proc`, or a Bun binding's real behavior. Such a test stays in its ordinary sibling `__tests__/` directory (no subdirectory, no filename suffix — `packages/server/src/__tests__/real-fs-invocation.test.ts` pins the sibling-mapping reason), calls `assertRealFs(label)` (`packages/server/src/__tests__/utils/memfs-detection.ts`) in every such case instead of skipping, and is added to BOTH halves of `package.json`'s `test` script — once as an exclusion via `--path-ignore-patterns` in the first invocation, once as an explicit path in the second. It must never import `mock-fs-helper.ts`. Everything that CAN pass under memfs stays in the first invocation. Use one `--path-ignore-patterns` flag per excluded file, never a comma-joined value — bun 1.3.14 does not split a comma-separated `--path-ignore-patterns` argument into multiple patterns; it is parsed as one literal glob that matches nothing, which silently excludes zero files.

## Scratch Git Repositories

A test or smoke that creates a throwaway git repository and commits inside it must use `createScratchGitRepo` (`packages/server/src/__tests__/utils/scratch-git.ts`), never a hand-rolled `git init`/`git commit` sequence. Without it, the commit inherits the operator's own global git config (signing program, `core.hooksPath`, `init.defaultBranch`, aliases) via the ordinary git config resolution chain, so the test's outcome depends on host state it never declared -- see `os-environment-coupling.md`. In particular, a host with commit signing enabled (`commit.gpgsign = true` and a real `gpg.ssh.program`) fails the commit before the code under test is ever reached, producing a false red with zero billed turns for a billable smoke.

The helper writes its own throwaway global config and points `GIT_CONFIG_GLOBAL` at it for the scratch repository's own commands only -- it never touches `git config --global` or the operator's real `~/.gitconfig`, and its structural guards make it impossible to aim the scratch config at anything but a directory the helper itself just created under a temp root. The guards protect *where the helper creates the repository*; they do not police how a caller subsequently uses the returned `repo.env`. A caller that spawns its own git command using `repo.env` must keep that spawn's `cwd` at `repo.dir` -- pointing `repo.env` at a different `cwd` (a real checkout, a different scratch dir) is not something the helper detects or prevents. Never write `-c commit.gpgsign=false` or `git config --global` directly in a test or smoke as a substitute. This does not collide with the standing rule that commits in this repository are never made with signing disabled: that rule governs commits that can reach a real PR, and a scratch config applied only to a directory the helper itself just created under a temp root -- never inside the real repository, per the helper's own guards -- is a different object. Plain Node scripts that cannot import TypeScript (e.g. `.mjs` test files) may set `GIT_CONFIG_GLOBAL`/`GIT_CONFIG_NOSYSTEM=1` inline for their own git spawns, with a comment naming `scratch-git.ts` as the single writer of the TS-importable version of the same pattern.
