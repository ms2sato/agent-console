...
2. **Run type check:** Execute `bun run typecheck` and ensure no type errors. The script auto-generates `routeTree.gen.ts` (TanStack Router) via `vite build` if the file is missing, so it runs correctly on a fresh worktree.
   ...
   - **Stale `schema-version.gen.ts` caveat (same shape, different generator):** `SCHEMA_VERSION` is a content hash over `packages/shared/src/schemas/*.ts`. After modifying anything under that directory, regenerate it (`bun run build`, or `node scripts/generate-schema-version.mjs`) before reporting done. A stale constant passes `bun run typecheck` and passes every individual test file you are likely to run for your own change — only the FULL `bun run test` catches it, via `schema-version.gen.test.ts`'s staleness check. Two independent delegates hit this on the same day (Sprint 2026-08-18, PRs [#1356](https://github.com/ms2sato/agent-console/pull/1356) and [#1357](https://github.com/ms2sato/agent-console/pull/1357)), each after running typecheck plus their own test files and finding them clean. **When delegating schema work, put the regeneration step in the delegation prompt** — the delegate has no reason to suspect a generated file they never opened.

     **A merge conflict is the second door into this same trap, and the more dangerous one**, because a conflict *demands* a decision and both available answers are wrong. `SCHEMA_VERSION` is a content hash, so when two branches both touched `packages/shared/src/schemas/`, neither side's value describes the merged file set — **the correct value is a third one that appears on neither side**. Resolve it by regenerating, never by choosing. Choosing lands you in exactly the stale state above, arrived at through a door the paragraph above does not watch: it passes `typecheck` and every test you would run for your own change.

     **The tell is an observable, not a recollection: the regenerated hash differs from BOTH sides. If it matches either one, something is wrong** — either the regeneration did not run, or the schema files are not in the state you think they are. This is the check the instruction "regenerate it" was missing; without it, "I regenerated" is a memory, and a memory is what fails under conflict pressure.

     **The tell is scoped to the conflict case, where both sides moved.** In a merge where only one side touched `packages/shared/src/schemas/` there is no conflict and the constant has one correct value — so **a match with that side is the expected result**, not a warning. Run the generator there anyway: the point is never to reason about whether the hash could have moved, only to look at what it is. (Worked example, Sprint 2026-08-29: a delegate merged a docs-only `origin/main` into a schema-touching branch, regenerated rather than reasoning about it, and got `a9b559256e2b7e37` both committed and regenerated. A match, and correct — the case where the reasoning would have been right and running the generator cost nothing to confirm it.)

     **A third door opens with no conflict at all:** a PR whose hash is correct at review time goes stale when a sibling merge widens the input set or changes a file inside the runtime-import closure, and because nothing marks the branch dirty and the merge-ref CI run is the only signal — on a surface nobody re-reads after review — the gate for this door is the pre-`gh pr merge` check in [`core-responsibilities.md`](../skills/orchestrator/core-responsibilities.md) §6, not anything you can see from the branch. (Worked example, 2026-09-04: [#1601](https://github.com/ms2sato/agent-console/pull/1601) widened the hash input to the runtime-import closure; [#1602](https://github.com/ms2sato/agent-console/pull/1602), reviewed clean with its hash verified equal to the `main` of that hour, added a constant to `packages/shared/src/types/embedded-agent.ts` — a file inside the closure only after #1601. The merge tree's regenerated value was `9864a6a1095bf63a`, neither side's.)
...
8. **Public-artifact language check:** When adding or modifying any file under `docs/`, `.claude/`, `scripts/`, or `CLAUDE.md`, run `bun run check:lang` and ensure exit 0. The check rejects any non-Latin Letter character — see Language Policy below. The same check also runs as part of `node .claude/skills/orchestrator/preflight-check.js` and the `language-lint` CI workflow.
...
**The same discipline applies one step earlier, to the query that produces a refutation.** Before refuting a count claim ("the string appears twice"), match your pattern to the claim's own stated method: "appears" means substring, not a code-shape regex. A narrower instrument's non-empty result is not the claim's count, and it fails in the opposite direction from the empty result above — it produces a confident wrong refutation instead of a confident wrong absence. (Lesson: Sprint 2026-09-04, PR #1612 — a grep for `\.model\|model:` "refuted" a PR body's "appears twice, once in a comment, once as code"; it had silently excluded the comment's "some Ollama **models**". The plain-substring grep confirmed the body, and the refutation was retracted in the same turn.)
...
- **TDD for bug fixes:** Write a failing test first, then implement the fix. Verify the test's polarity by stashing **only the candidate fix** while keeping the new test present — either commit the failing test first and `git stash` the fix, or use `git stash --patch` to deselect the test hunks. Run the new test against unmodified production code (must fail), then restore the fix and confirm it passes. Tests that pass in both directions (with and without the fix) are not actually testing the bug; they will continue to pass after a future regression. (Lesson: Sprint 2026-04-29 — recurring agent pattern of writing tests that exercise adjacent code instead of the changed line.)
  ...
  **Every pin's reach is measured, not predicted.** Polarity asks whether a test distinguishes the two worlds; reach asks **which mutations it actually catches**, and polarity is the special case where the mutation is "remove the fix". Before believing an assertion guards what you think it guards, **mutate the thing it guards and watch it fail.** Predicted reach is wrong in both directions often enough that prediction is not evidence: across **one PR's fifteen measured mutations** ([#1432](https://github.com/ms2sato/agent-console/pull/1432)), two pins caught more than predicted, two caught fewer, and three caught nothing at all and were rebuilt. Record the measurement in the test's own comment, so the next reader inherits the reach instead of re-deriving it.

  **Pins include type-level assertions, and that boundary is the one that gets missed.** A type pin's mutation is: introduce the drift it exists to catch, run `tsc`, observe the diagnostic. **A pin that type-checks under its own drift is inert**, and it is easy to write one, because `declare const x: never` is legal TypeScript — `declare` introduces no assignment, so `never` has nothing to reject:

  ```ts
  type _A = SchemaOut extends TheType ? true : never;
  declare const _assertions: _A;                              // drift present -> tsc exit 0, no diagnostic

  type Assert<T extends true> = T;
  type _A = Assert<SchemaOut extends TheType ? true : false>;  // same drift -> error TS2344, exit 2
  ```

  Both halves are load-bearing: `never` → `false` alone still passes, and the constraint alone still admits `never`. (Instances and full repro: Issue [#1442](https://github.com/ms2sato/agent-console/issues/1442).)

  This requirement was previously written down **once**, in [`test-trigger.md`](./test-trigger.md)'s graceful-exit note, scoped to a single smoke script — where a reader looking for the general discipline would never find it. That is why its category boundary was invisible: a rule that exists in only one instance has no visible scope to check yourself against, so "pin" silently meant "test-shaped" and type-level assertions fell outside it.
...
**Scope extension: the same trap fires on any tool input body, not only commit messages for elevation code.** PR bodies, Issue bodies, `gh api ... -X PATCH` heredoc contents, and any other narrative body containing the `sudo` substring — even inside a quoted test name (e.g. `describe('sudo-skip (direct) path')`) or narration referencing an existing identifier — will be blocked identically. The trap is content-based, not context-aware. When a public artifact needs to reference such an identifier, paraphrase the reference (`describe('elevation-skip (direct) path')` in the body reference, or `elevation-skip test in multi-user-mode.test.ts`), leaving the source file's actual name unchanged. This applies regardless of whether the PR itself touches elevation code — it triggers whenever the narrative substring appears. **It also fires on file content, not only on command-line bodies**: a heredoc that writes a memo, a scratch note, an acceptance-check answer file, or a Python snippet whose string literal names the elevation command, the dot-env filename pattern, or a recursive/force delete flag is blocked identically, and a subagent's "I wrote the file" may then describe a file that does not exist. Verify the write with a listing; paraphrase inside file content the same way as in bodies. (Observed three times in Sprint 2026-09-09..13, on a memo heredoc, a scratch cleanup, and a memory append.)
