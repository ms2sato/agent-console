...
2. **Run type check:** Execute `bun run typecheck` and ensure no type errors. The script auto-generates `routeTree.gen.ts` (TanStack Router) via `vite build` if the file is missing, so it runs correctly on a fresh worktree.
   ...
   - **Stale `schema-version.gen.ts` caveat (same shape, different generator):** `SCHEMA_VERSION` is a content hash over `packages/shared/src/schemas/*.ts`. After modifying anything under that directory, regenerate it (`bun run build`, or `node scripts/generate-schema-version.mjs`) before reporting done. A stale constant passes `bun run typecheck` and passes every individual test file you are likely to run for your own change — only the FULL `bun run test` catches it, via `schema-version.gen.test.ts`'s staleness check. Two independent delegates hit this on the same day (Sprint 2026-08-18, PRs [#1356](https://github.com/ms2sato/agent-console/pull/1356) and [#1357](https://github.com/ms2sato/agent-console/pull/1357)), each after running typecheck plus their own test files and finding them clean. **When delegating schema work, put the regeneration step in the delegation prompt** — the delegate has no reason to suspect a generated file they never opened.
     ...
     **A merge is the second door into the same trap.** When both branches touched the hash's input set, neither side's `SCHEMA_VERSION` describes the merged tree — the correct value is usually a third one, so resolve by regenerating, never by choosing. The regenerated value is compared with the MERGED input tree, not with either branch: a mismatch with both sides is the ordinary outcome, and a match with one side is a prompt to confirm the generator actually ran on the merged files rather than proof that it did not (two branches can edit the inputs to identical final content, and then the values legitimately coincide). When only one side touched the inputs, a match with that side is the expected result — run the generator anyway, because the point is to look at the value, not to reason about whether it could have moved. (Sprint 2026-08-29: a docs-only merge into a schema-touching branch regenerated to the same `a9b559256e2b7e37` — correct, and free to confirm.)
