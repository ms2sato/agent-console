...
8. **Signature shape change pre-estimate — for PRs that change a function / method signature shape:**
   ...
   **8.5 (review-mechanism ceiling) — for any PR that must land atomically:** the same count has a second reader. **CodeRabbit skips a PR over 100 changed files entirely**, and this repository's plan allows one review per hour (see the [`coderabbit-ops`](../skills/coderabbit-ops/SKILL.md) skill). A PR that cannot be reviewed cannot satisfy the Definition of Done, and no fallback disposition substitutes for a review that never started.

   **So estimate the file count while writing the AC, not at the gate**, and if it approaches the ceiling, choose the mitigation there: a preceding PR that removes mechanical fan-out, or a stacked review whose merge is still one move. **Do not split the atomic change itself to fit a tool** — when the atomicity was a design decision, the window it protects is measured in review round-trips, not minutes.

   (Lesson: Sprint 2026-08-28 PR [#1403](https://github.com/ms2sato/agent-console/pull/1403) — 107 files against a cap of 100, discovered at the gate rather than at AC time, and reported by the commit status as `Review rate limited`, which reads as "wait" for a condition that would never clear. Twelve of those files were migration tests whose whole diff bumped an assertion of the schema's terminal version — a fact another test already owned. Removing that duplication was correct on its own merits, took the PR to 95, and let a real review run, which then found a Major that both the implementation and an independent audit had missed. The Architect recorded the missing AC-time check as their own defect.)
