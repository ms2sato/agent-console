...
13. **Role-switch self-pass — for anyone ISSUING an AC / verification plan / experiment design, and for anyone AUDITING one:**
    ...
    **And ship the AC's `- [ ] ` checklist in the same message as the AC, never as a follow-up.** `acceptance-check.js` reads `^- [ ] ` lines and nothing else; an AC posted as prose rulings makes Q3 report "No acceptance criteria (checklist) found" and silently degrades the criterion-to-test mapping to the Orchestrator's judgment. When the AC is prose, its author converts each required item into a checklist line before posting, in the same comment — not "the Orchestrator will derive one" and not "I'll send it if asked". (Lesson: Sprint 2026-09-09, #1621 — the posted ruling had zero checklist lines; the checklist arrived as a second message after the acceptance check had already run without it.)
