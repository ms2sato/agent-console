...
6. **Layer-Boundary Crossing Checklist — for PRs that introduce a cross-runtime spawn:**
   ...
   **This checklist also applies whenever a spawned script GAINS a new npm dependency it did not previously have, even when the spawn itself stays same-runtime** — "does every caller workflow install dependencies" belongs here regardless of whether the spawn crosses a runtime boundary. PR #1598 added the `typescript` package (via a transitive import) to a script already spawned same-runtime (`bun` invoking a `bun` script) by an existing caller workflow, `.github/workflows/preflight-check.yml`, which had no `bun install` step because the checks it called had zero npm dependencies until then — the caller crashed with the same empty-stdout/exit-1 false-"0 violations" shape as the PR #716 cross-runtime lesson above.
...
10. **Schema-Type Parallel Maintenance — for PRs that add a derived/computed field to a shared type that crosses ANY boundary where the type has a valibot schema pair:**

    When this PR adds a field to a shared TypeScript type (`packages/shared/src/types/`) that is parsed by a `valibot` schema on the other side of a boundary, walk this 3-step checklist. The server↔client wire (WebSocket or REST) is the boundary this question was written for, and the lessons below are about it, but it is **one of at least three** in this repo: the server↔embedded-agent subprocess boundary (stdin NDJSON parsed by `EmbeddedAgentCommandSchema` in `packages/embedded-agent/src/main.ts`) and the server↔MCP client boundary carry the identical failure. Sprint 2026-09-02 (#1554) nearly shipped a `reasoningEffort` field added to `types/embedded-agent.ts` without the matching entry in `schemas/embedded-agent.ts`, which would have failed activation of every override-carrying worker at the subprocess boundary — a larger blast radius than the client case below. The structure invites it: the TS type and the schema live in same-named files in sibling directories (`types/` and `schemas/`), so editing one feels like editing both. The mechanical opener is the delegate's own phrasing from #1561: at the moment you edit the type, grep for where it is `v.safeParse`d.
    ...
    **What the failure is now (re-derive, do not trust):** since Issue [#927](https://github.com/ms2sato/agent-console/issues/927), every wire schema uses `v.strictObject`, which *rejects* a payload with an unknown field instead of stripping it, and `parseMessage` in `packages/client/src/lib/app-websocket.ts` drops a frame that fails `safeParse` with only a browser-console `logger.error`. Forgetting step 1 therefore discards the **entire message**, and the only trace is on a surface no test or server log observes — which is why step 2's integration test, not a unit test, is the layer that catches it. Both facts are properties of code that a future migration can change again; re-read them in seconds with `grep -ohE 'v\.[a-zA-Z]*[Oo]bject\(' packages/shared/src/schemas/*.ts | sort | uniq -c` (count the constructors present — a zero for `v.object(` is satisfied by a third constructor or by no schemas at all) and the failure branch of `parseMessage`.

    **A picklist (or other accepted-vocabulary) source living outside `packages/shared/src/schemas/` is itself a Q10-class miss, even when no schema FILE's text changes.** A schema's `v.picklist(SOME_CONSTANT)` reads its accepted wire values from wherever `SOME_CONSTANT` is defined; if that definition lives in `packages/shared/src/types/` and changes, the schema's accepted values change with it, but nothing under `schemas/` was touched. `scripts/generate-schema-version.mjs` now hashes the transitive runtime-import closure of the schema files (not just the schema files themselves) specifically so the tell for this class of miss is `SCHEMA_VERSION` moving, not a schema file's diff. `PTY_NOTIFICATION_KINDS` (`packages/shared/src/types/system-events.ts`), imported at runtime by `packages/shared/src/schemas/embedded-agent.ts`'s picklist, is the concrete instance this closure was built to catch — Issue #1580.
...
12. **Is the AC's own verification requirement still satisfiable? — before starting implementation:**
    ...
    4. **Name the observable the AC's verification reads, and show it exists in the code at the candidate before billing or building against it.** "Runnable" (steps 1 to 3) asks whether the check can execute; this asks whether the thing the check reads is there at all. An AC written from a design doc, a sibling's description, or a memory of the code will name an observable that the code does not expose, or a caller that no longer exists, and every step of a well-built verification then measures something else. The cheap check is a grep at the candidate SHA for the observable's name (a route field, a header, a function's callers), run by the AC author at issuance and repeated by the implementer before the first billable or hard-to-reverse step. Record the grep in the AC as the evidence the observable exists, the way step 2 records that the verification runs. When the grep finds nothing, that is a finding about the AC, and it is cheaper before implementation than after.

    (Lessons, three in one QA line, 2026-09-09, all found by the delegate before billing and none by the AC's author. The #1619 AC assumed the seven real-host smokes could run inside the verification container because it had the users and the elevation rules; it had no source tree — the runtime stage copied only `dist/`, and the fix (`/workspace`) doubled the image and needed a ruling. The #1632 AC told REST delete routes to resolve the delete trigger's session "the way the MCP handlers do", from the record with the calling session as the fallback; REST has no calling session, so the fallback the AC named does not exist on that path. The #1622 AC ruled that both the restart path and the session-edit path thread the spawn user through the rename, and specified a sub-check driving the session-edit route; that route no longer accepts a branch, the whole path had no live caller, and the correct fix was deletion. Each AC named an observable or a caller as a fact about the code, and the fact was about a document or an earlier version. Cross-reference from Q13's "Production-real or proxy?": a proxy for an observable that does not exist is not a proxy, it is a fabrication, and Q13's recorded-proxy discipline cannot rescue it.)
...
13. **Role-switch self-pass — for anyone ISSUING an AC / verification plan / experiment design, and for anyone AUDITING one:**
    ...
    **And ship the AC's `- [ ] ` checklist together with the AC, never as a follow-up.** `acceptance-check.js` reads `^- [ ] ` lines from the Issue BODY and nothing else — not comments, not messages — so the Orchestrator transcribes the AC and its checklist into the body at dispatch (`orchestrator/SKILL.md` "The AC goes in the Issue BODY"). What this paragraph adds is the author's half: an AC delivered as prose rulings makes Q3 report "No acceptance criteria (checklist) found" and silently degrades the criterion-to-test mapping to the Orchestrator's judgment, so the author converts each required item into a checklist line in the same delivery as the AC — not "the Orchestrator will derive one" and not "I'll send it if asked". (Lesson: Sprint 2026-09-09, #1621 — the delivered ruling had zero checklist lines; the checklist arrived as a second message after the acceptance check had already run without it.)
