...
**Only ABSENCE assertions are vulnerable to being read too early, and that is what makes the sweep possible.** A presence check (`some(...)`) read early merely retries — it cannot pass falsely. An absence check (`!some(...)`) read early passes *because the thing has not happened yet*, and passes identically whether or not it was going to. So an absence assertion is only as strong as the latest moment it could observe: **snapshot it after the boundary past which the event would no longer be written** (a turn's `idle`, a replacement's `ready`), never at the first sign the turn produced.

This matters more than it sounds, because "reads too early" is a judgement and cannot be enumerated, while `!...some(` is a grep. Define the class by polarity and a file's absence assertions can be listed and checked one by one.
