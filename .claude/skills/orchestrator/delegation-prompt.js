...
5. Create PR with a **conventional-commit title** — the squash merge reuses it as the commit subject on \`main\`: \`type(scope): <what changed> (closes #${issueNumber})\`, e.g. \`fix(client): place the flag below the activity dot (closes #${issueNumber})\`. Do NOT paste the Issue title as-is and do NOT use any \`[AI] closed #N ...\` form — that shape was this template's own wording until 2026-09-13 and produced two retitle round trips in one day. The body carries \`Closes #${issueNumber}\`. **If Issue #${issueNumber} is a sub-issue of a parent** (its body declares \`Part N of #parent\` or the orchestrator's notes mention multi-slice delivery), the **3-layer cleaning rule** applies — PR title must use \`type(scope): <what changed> (Part N of #parent)\` form (NO close keyword for the parent), PR body uses \`Closes #${issueNumber}\` + \`Refs #parent\` only, and the original commit message uses \`Refs #parent\` only. GitHub's auto-close parser scans title / body / commit message; any parent close keyword in any of the three auto-closes the parent on squash merge. See \`core-responsibilities.md § Multi-PR Delivery (Sub-Issue Pattern)\` for the full rule.
