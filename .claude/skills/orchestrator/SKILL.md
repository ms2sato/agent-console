...
2. **Orchestrator-designation self-flag.** Call the `set_orchestrator_session` MCP tool for your own session at startup, so that a generation change (a fresh Orchestrator session replacing a prior one) re-flags automatically for labeled-Issue webhook routing (Issue #1643).
