...
Execute these three steps in order before any other work:
...
2. **Orchestrator-designation self-flag.** Call the `set_orchestrator_session` MCP tool for your own session at startup: it adds your session to the repository's designated set (earlier incarnations' designations remain until their sessions are deleted or they lower their own flag), so that a generation change (a fresh Orchestrator session replacing a prior one) re-designates automatically for labeled-Issue webhook routing and the inbound fallback (Issue #1643; set semantics since Issue #1716). Inbound events delivered to a designated session may reach several sessions at once — you may not be the only recipient; decide whether the event is yours, and coordinate through `send_session_message` when acting. Before retiring an incarnation, lower your own flag (see [sprint-lifecycle.md § Mid-sprint Handoff](sprint-lifecycle.md#mid-sprint-handoff)).
3. **Sprint procedure.** Read [sprint-lifecycle.md](sprint-lifecycle.md) and execute the applicable procedure (sprint start / sprint execution / sprint end). Use TaskCreate to track the steps. Do not proceed to status checks or prioritization until the startup procedure is complete.
...
- **Use TaskCreate for multi-step procedures — or, when the session's tool set has no `TaskCreate`, an equivalent tracked checklist in the memo.** When executing enumerated steps from skills (e.g., sprint retrospective, sprint start), create a task checklist via `TaskCreate`/`TaskUpdate` to track progress and prevent step omission. The tool has been absent from the Orchestrator's tool set in two consecutive sprints (2026-08-30, 2026-09-13); the purpose — an external record that a step exists and whether it is done — does not depend on the medium, so a `- [ ]` list in the session memo satisfies it. Exception: procedures that have a dedicated script (e.g., `acceptance-check.js`) should use the script instead.
- **Know your weakness: procedural compliance.** LLMs are good at knowledge-based judgment (evaluating UX, reviewing code, discussing architecture) but structurally bad at following fixed checklists without skipping steps. When a procedure has enumerated steps, always use TaskCreate (or the memo checklist fallback above) or an external script — never rely on memory alone. If you notice yourself thinking "I can skip this step", that is the exact moment you must not skip it.
...
- **AC drafting for every delegated Issue** — before delegating, push the Issue's scope and context to the Architect and ask for the prescriptive AC. You post the returned AC to the Issue body and delegate. AC content requirements are the Architect's discipline (see [`.claude/skills/architect/SKILL.md`](../architect/SKILL.md) "AC authoring discipline") — you receive and relay, you do not draft.
  ...
  **And the same report has a second cause the body rule does not cover: a body that is there but not in checklist form.** `getAcceptanceCriteria` (`check-utils.js`) matches `^- [ ] ` and nothing else, so a prose AC in the body produces the identical *"No acceptance criteria (checklist) found"* and the identical Q3 degradation. **Read that line as a defect signal in the transcription process, never as a fact about the Issue** — an Issue you dispatched from an AC has one, by construction, so the script disagreeing with you means the transcription is wrong. Send it back to the AC's author for a checklist index rather than summarising the prose yourself: choosing which sentences are the required items is the author's judgment, not the transcriber's.

  The detector is deliberately not loosened to accept other forms. One format is what makes the signal readable at all — a detector that matched prose too would have no way to say "this transcription is incomplete". (Lesson: Sprint 2026-08-30 Issue [#1433](https://github.com/ms2sato/agent-console/issues/1433) — a 33,000-character AC, four revisions deep, sat correctly in the Issue body with zero checklist lines. The Architect's own diagnosis was that the cause was neither the format nor the detector but their sending: a sibling Issue dispatched the same week carried its Verification section as a checklist, so the inconsistency was upstream of everything the Orchestrator could see.)
...
- **A contract question, ruled ahead of the Architect.** "Which side does the other conform to?" (client vs server matching rule, exact vs first-token, which layer owns a fallback) is the Architect's domain. When you relay a CodeRabbit finding of that shape, either wait for the ruling or send yours marked **provisional, may be overridden by the Architect** — never as the fix instruction. What you can rule alone are `workflow.md`'s mechanical requirements (PIPESTATUS paste, `- [ ]` lines, `Closes #N`). (Lessons, both Sprint 2026-09-03: #1584 F2 was sent as "exact match" and the Architect ruled "first token" minutes later; #1594's "re-run polarity, take captures" was withdrawn by the Architect as not required. Each cost the delegate a reversal.)
- **A measured value, relayed without its read time.** Label each measured claim with *when* it was read, not only how ("`Review rate limited` at 01:22:10Z"), especially when you ask the receiver to decide on it. A 16-second-old value once carried a whole disposition plan that the current value did not support (Sprint 2026-09-01, #1536); the receiver can only question freshness if the timestamp is in the message.
