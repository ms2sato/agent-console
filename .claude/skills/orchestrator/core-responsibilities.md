...
- **Separate symptom from hypothesized root cause in the Issue body.** Issue creators (orchestrator) typically file before reading the affected code in depth, so root-cause statements at filing time carry low confidence by construction. Mixing observation and conjecture into a single "what's wrong" paragraph routes downstream agents to the wrong fix scope. Structure the body in three blocks instead:
  1. **Symptom** — observed behaviour, reproduction steps, screenshots / log excerpts. No interpretation. Anything a non-author would also see.
  2. **Hypothesized root cause** — which layer (server / client / shared contract / persistence / event protocol / external integration) you suspect, with explicit confidence (high / medium / low) and the reasoning. Mark as hypothesis, not conclusion.
  3. **Layer candidates for investigation** — list every layer the fix could plausibly touch. At low confidence, list all; at high confidence, list the narrowed set with the elimination reasoning. The agent picks the actual fix scope after their own investigation.

  This structure preserves the agent's investigation room rather than anchoring them to the orchestrator's premature framing. (Lesson: Sprint 2026-05-03 — Issue #762 was filed with a single "client-side replay is the problem" paragraph that conflated symptom with conjecture; the owner's "isn't it actually fetching the full thing?" pushback and the agent's deeper investigation in #769 surfaced the real root cause as a server-side offset semantic mismatch, an entirely different layer. The originally-framed scope would have produced the wrong fix.)
- **Removal-type AC must grep the existing surface first.** When an Acceptance Criterion asserts a symbol / field / behavior is absent, should be removed, or "is not present", grep the repository for that symbol before writing the criterion. When a design document rejects an alternative phrased as "the alternative — A and B — is rejected", read the reject target as the **(object, alternative) pair**, not as a standalone instruction to delete A or B individually. (Lesson: Sprint 2026-05-12 Issue #774 / PR #778 — the AC was written to remove `createdBy` from the session payload, but the design doc rejected exposing the *set of shared-account user-ids*, not per-session `createdBy`; `createdBy` was already used by `SessionBase` and a grep would have surfaced the contradiction. The owner caught it.)
...
- **Sweep / diff-scoped task instructions must include the full file list verbatim.** When delegating any task whose scope is "the diff of this PR" (comment sweep, doc-rewrite sweep, format pass, dead-code removal pass, etc.), do NOT write an in-scope file list by hand. Instead, run `git diff --stat origin/main...HEAD` (or the equivalent for the agent's branch) and paste the full output verbatim into the delegation prompt as the authoritative scope. Hand-written lists routinely omit files, leading to inconsistent application of the sweep rubric and follow-up clean-up commits. The same `--stat` output also lets the agent verify they have not missed a file in their own pre-push enumeration. (Lesson: Sprint 2026-06-26 PR #897 — the Orchestrator's comment-sweep instruction listed 5 of 6 changed files, omitting `lib/__tests__/git.test.ts`. The agent's subagent honestly reported the miss in finishing notes, triggering an additional sweep commit. Pasting `git diff --stat` would have prevented the omission entirely.)
...
- **E2E-by-default in delegation.** When delegating a change with observable runtime behaviour on a reachable path, instruct the agent that a shipping-path E2E is the default and that skipping it is a joint decision (agent must consult the Orchestrator / owner at that point, never silently skip and open the PR). See `workflow.md` Testing Requirements "E2E / real-device verification is the default".
...
- **CI-completion wakeup conditions must not silently stay false.** Do not gate a `create_conditional_wakeup` on "all checks COMPLETED" — the `CodeRabbit` entry in `statusCheckRollup` can stay `status: null` indefinitely (rate-limit / docs-only skip), making the condition false forever even when CI is green. Enumerate the required workflows by name instead. Never compare a display-truncated SHA against `gh`'s full 40-char `headSha` (expand with `git rev-parse`, or use `startswith`). A structurally always-false condition produces a false timeout, not a real CI signal — verify any timeout against `gh run list` / `gh pr view` primary info before acting. (Lesson: Sprint 2026-05-12 PR #789 — two consecutive false timeouts from a CodeRabbit-null condition and a truncated-SHA comparison while CI was actually green.)
...
### Delegation Prompt Mandatory Checklist

Every delegation prompt sent via `delegate_to_worktree` must explicitly include the four items below. Missing any one of them turns into a recoverable gap at best and a silent failure at worst; the Sprint 2026-06-30 retrospective surfaced cases for all four.

1. **Post-merge retrospective request.** The prompt must instruct the agent to send a retrospective report together with the merge confirmation. Without this instruction, the agent reports the merge and the Orchestrator may proceed to `remove_worktree` (which deletes the session) before any retro arrives — losing it permanently. Use phrasing such as: *"After your PR is merged, send a retrospective via `send_session_message` covering: (1) what went well, (2) friction points and how you resolved them, (3) proposed improvements to agent definitions / skills / rules, (4) feedback for the Orchestrator. Do this before standing down so the worktree cleanup happens after the retro is received."* See `sprint-lifecycle.md` "Retrospective Collection and Process Improvement" for the order.

2. **Integration-test requirement decision.** When the change touches both client and server packages (or otherwise crosses a wire boundary — WebSocket message, REST request/response, shared type with derived fields), the prompt must explicitly require an integration test in `packages/integration/src/` rather than leaving it to the agent's judgment. A "judgment OK to skip" reached jointly by the agent and the Orchestrator does not unlock the strong preflight recommend — see `workflow.md` "A joint decision does not unlock a strong preflight recommend." If a skip is genuinely defensible (the change is purely a behavior subtraction with no new wire field, etc.), the prompt should say so explicitly and cite which `workflow.md` "Skip threshold" condition applies; otherwise the integration test is in scope.

3. **Browser QA bridge — where it lands.** When the change has user-observable UI behavior, the prompt must specify where Browser QA will be performed: in the delegated session (if Chrome DevTools MCP is reachable there), in the Orchestrator session, or via owner dogfood. Delegated sessions on this codebase typically lack Chrome DevTools MCP; do not leave this to the agent to discover mid-task. State up front: *"Browser QA will be performed by [owner / Orchestrator]; you do not need to take screenshots in your environment. Document the QA repro steps in the PR body so the QA gate has a precise script."* When the bridge is to the owner, ready the PR for review at push time (not as a draft) so CodeRabbit and owner QA proceed in parallel — see `feedback_no_draft_when_requesting_owner.md`.

4. **Delegated session environment constraints.** Include a short preamble listing the constraints the agent is likely to discover the hard way otherwise: no Chrome DevTools MCP, no sudo (so commands that require root must be flagged for the owner or Orchestrator to run), the dev:multiuser `WorkingDirectory` is rsync-snapshotted at script start (so edits in the agent's worktree do not propagate live to the running dev server — see `dev-environment-quirks` skill), and `/home/agentconsole/` may not be readable from the agent's `ms2sato` shell (permission-denied looks identical to file-not-found if stderr is suppressed — see `development-workflow-standards` "Diagnostic Command Error Suppression"). Listing these up front prevents one to two round trips of "I tried X, got Y, why?" early in the task.
...
- **CI Green + CodeRabbit Complete -> Acceptance Check Flow**:
  0. **Prerequisite: CodeRabbit review must be complete.** Establish that by walking the verdict-surface checklist in the [`coderabbit-ops`](../coderabbit-ops/SKILL.md) skill, which is its single writer. **Do not read it off `gh pr checks`** — that command prints the word `pass` for a CodeRabbit row whose description says `Review rate limited`, i.e. for a commit the bot never looked at. If CodeRabbit is pending or rate-limited, wait for it before starting the acceptance check, or apply the documented fallback. Do NOT merge a PR without a completed CodeRabbit review.
  1. Start the acceptance check via `run_process` (see above). Answer Q1-Q12 via `write_process_response`. If the script reports `[No linked Issue]`, instruct the agent to add `Closes #NNN` to the PR body before proceeding. Do not ignore this warning.
...
- **MANDATORY: Merge execution gate — read `mergeStateStatus` immediately before merging, and merge only through a conditional check.** Never issue the merge as an unconditional command on the strength of an earlier "CI looked green" observation:
  ...
  ```bash
  status=$(gh pr view <N> --json mergeStateStatus --jq .mergeStateStatus)
  if [ "$status" = "CLEAN" ]; then gh pr merge <N> --squash; else echo "NOT CLEAN: $status - do not merge"; fi
  ```

  Any non-CLEAN value (`UNSTABLE`, `BLOCKED`, `BEHIND`, `DIRTY`, `UNKNOWN`) means a check is unfinished, failing, or the branch is stale — merging on it is a bet that the rollup completes green. (Lesson: Sprint 2026-08-01 PR #1265 — the merge fired while the rollup still showed UNSTABLE; the result happened to be green, but the ordering violation is exactly what the conditional form makes impossible. This gate is the codification of that correction.)
