...
- **Step 7 (write)** instructs the Orchestrator to create a TaskCreate task (or, when that tool is unavailable, a memo checklist line) pinned to the retro PR number so the deferred sync is not lost between conversation compactions.
- **Step 8 (verify, mechanical)** diffs the sprint window's merged PRs against a KNOWN set (`SPRINT_PR_NUMBERS` plus the retro PR's own number) by piping the window straight into `sprint-retro.js --gap-candidates <retro-pr-number>` — a client-side `mergedAt` filter, never `--search`, since `gh pr list` ignores the search query's date qualifiers unconditionally regardless of `--json` or `--state` (see Step 8's own instructions for the measurement and the working alternative). The gap-candidates mode builds the KNOWN set and diffs it through `sprint-retro.js`'s own SPRINT_PR_NUMBERS parser and `computeGapCandidates`, so Step 8 never re-states that delimiter contract in shell. This catches post-retro follow-up improvement PRs, brewing-log batch PRs, and hot-fix PRs that landed after Step 5 — none of which are in Step 7's narrow retro-PR scope. Every diff survivor is a CANDIDATE requiring a one-line disposition, not an automatic gap — the known set is a hypothesis at Step 8 execution time, and a per-PR grep against the memory files (which record a sprint as a range summary, not a per-PR ledger) misreads nearly the whole window as missing. The recurring, explainable candidate is the PREVIOUS sprint's own retro PR, caught by the window's inclusive lower bound; the memory-file grep applies only to the remaining candidates, never to every merged PR. Step 8 converts "did I remember everything?" (LLM-weak) into "scan + diff" (mechanical).
