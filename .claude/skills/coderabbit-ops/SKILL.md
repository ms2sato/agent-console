...
**The scoped form IS covered now, and it was not for ten months.** `docs(embedded-agent):`, `docs(orchestrator):`, `docs(workflow):` — the form most docs PRs here use — contains `docs(` but not `docs:`, so until the second keyword was added (PR [#1453](https://github.com/ms2sato/agent-console/pull/1453), closing [#1440](https://github.com/ms2sato/agent-console/issues/1440)) none of them was skipped. Such a PR competed for the quota exactly like a code PR, and did so invisibly, because its empty `reviewDecision` looks identical to the carve-out working.

**The discipline that outlives the fix: read the substring rule out of the config, never out of the convention it resembles.** In conventional-commits vocabulary `docs(scope):` **is** the docs prefix, so a reader who knows the convention will assume it matches — correct by that vocabulary, and true or false about this repository only according to what the YAML currently lists. The keyword list is two entries today; it is a config file and can be edited by anyone. `grep -A3 ignore_title_keywords .coderabbit.yaml` is the whole check.

(Lesson: Sprint 2026-08-29 — three docs PRs were merged that day on the reasoning "the `docs:` prefix means auto-review is skipped by configuration; an empty `reviewDecision` is the expected state rather than a wait condition", written into one of their bodies as a durable claim. All three were scoped titles, none was skipped, and all three read `Review rate limited` at surface 4 — the surface that would have refuted the claim in one query was on the same commit, saying something else. Measured with a control the same afternoon: retitling a PR from `docs(design-principles):` to a bare `docs:` moved its description from `Review rate limited` to `Review skipped: ignored keyword in the PR title`. Config fix landed 2026-08-30 in PR [#1453](https://github.com/ms2sato/agent-console/pull/1453), closing [#1440](https://github.com/ms2sato/agent-console/issues/1440); this paragraph is the reading discipline that holds whatever the config says.)

(Second lesson, Sprint 2026-08-31 — the three paragraphs above described the *pre-fix* config for a full day after that fix merged, asserting a consequence ("the scoped form is not covered") that the repository had already made false. Nobody re-asserted the claim, so no verification pass could reach it; this is `design-principles.md`'s "sweep for what your change invalidated" landing on a skill instead of a doc, and the tell was the one recorded there — **a rule citing an Issue as future work when that Issue is closed.** One `gh issue view 1440 --json state` settles it, and the paragraph itself named the Issue.)
...
**Draft is the one skip that is also a lever.** Opening a PR non-draft spends the hourly slot immediately, on whatever commit happens to be the head at that moment — which is why the queue is not the Orchestrator's to allocate by default: the auto-review fires on open, before anyone decides it should. A draft PR is skipped and **nothing is queued**, so the work can land, be revised, and be verified while the slot stays available for a PR that is actually ready. Marking ready is what starts the review, and that is a decision someone makes rather than a side effect of pushing.

The trap is the usual one wearing a fourth costume: `state` is `success`, so a draft that has never been looked at renders as `CodeRabbit=SUCCESS` in `statusCheckRollup` and as `pass` in `gh pr checks`, alongside genuinely green CI. **A draft PR is never "reviewed clean"** — it is unreviewed by construction, and only the `description` says so.

(Measured 2026-08-31 with a same-run control: PR #1529, opened draft, read `Review skipped: draft pull request` while PR #1523, non-draft, read `Review completed` on the same instrument at the same minute. The `allowance at 1 review` line in the walkthrough was unchanged by the draft's existence.)
