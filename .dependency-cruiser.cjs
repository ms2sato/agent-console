  ...
  forbidden: [
    ...
    // ───────────────────────────────────────────────────────────────
    {
      ...
      comment:
        ...
        'invert the direction. NOTE: the baselined ' +
        'packages/server/src/services/session-manager.ts <-> ' +
        'session-pause-resume-service.ts entry in ' +
        '.dependency-cruiser-known-violations.json only became visible after ' +
        'enabling `tsPreCompilationDeps` below (it is a type-only edge) -- ' +
        'the baseline JSON format itself is fully regenerated by ' +
        '`lint:deps:baseline` on every run and cannot carry a durable per-entry ' +
        'comment, so this rule-level comment is that entry\'s recorded home. ' +
        'Fixing it is tracked by Issue #1487; `madge`/`lint:cycles` drops that ' +
        'file from its graph entirely for a cause not yet established there.',
      from: {},
      to: { circular: true },
    },
    ...
    // ───────────────────────────────────────────────────────────────
    {
      ...
      from: {
        pathNot: [
          ...
          // api-client.ts's `import type { AppType } from '@agent-console/server/api-type'`
          // resolves via packages/server/package.json's `typesVersions` map
          // (a TypeScript-only resolution mechanism `tsc` understands natively).
          // dep-cruiser's enhancedResolveOptions reads a package.json `exports`
          // field, which this package intentionally does not declare, so this
          // edge reads as unresolvable once `tsPreCompilationDeps: true` makes
          // it visible to the graph at all. `tsc --noEmit` (run separately in
          // CI) is the authority on whether this type-only edge actually
          // resolves; not-to-unresolvable is not the right gate for it.
