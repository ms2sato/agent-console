...
### Notification Center
**Design only (spec: [notification-center.md](design/notification-center.md)); no implementation yet.** The **human**-addressed awareness surface: a bell in the header plus a list of recent events, so a person who was not watching can still find out what happened. It is a **composed read-model, not an event store** — `GET /api/notifications` builds the list from existing domain rows at request time (v1 sources: artifacts, `worktree:delete` jobs), and an event kind is admitted only once it already has a durable domain record of its own (the record/awareness cut, §2 and §4). Two invariants carry the design: **N1** — the list renders only from domain rows read at fetch time, so a WebSocket broadcast is a cache-invalidation hint and never itself rendered (a phantom broadcast degrades to a refetch that finds nothing, the structural answer to [#1330](https://github.com/ms2sato/agent-console/issues/1330)); **N2** — items carry no state of their own. v1 is **awareness-only and personal-scope** (both owner-confirmed 2026-08-18): obligations, which clear by *doing* rather than *seeing*, are a separate future lane, and the feed shows only events the viewing user owns or initiated.
