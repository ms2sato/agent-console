...
**Design phase (spec: [embedded-agent-sdk-engine.md](design/embedded-agent-sdk-engine.md), Issue [#1324](https://github.com/ms2sato/agent-console/issues/1324)).** The in-subprocess machinery that turns user messages into NDJSON events — loop, tool execution, and provider communication as one unit. Two engines exist as siblings behind the SAME host↔subprocess stdio/NDJSON protocol (the seam is that protocol, not a new interface): the **native-loop engine** (the existing `agent-loop.ts` + `providers/` + `tools/` + `mcp.ts` stack) and the **SDK engine** (hosts a Claude Agent SDK session; the executing user's own claude authentication, no provider secret crossing the server). The engine is NOT user-facing configuration — it is determined by an explicit internal discriminant on the [EmbeddedAgentDefinition](#embeddedagentdefinition) (exhaustive switch; field-presence inference prohibited), and users simply pick which agent they want. Key accepted trade (owner, 2026-08-17): an SDK-engine worker costs TUI-Claude memory (~450 MB), not custom-loop memory (~92 MB), in exchange for removing the subscription-vs-API-key configuration fork.
- **See:** [embedded-agent-sdk-engine.md](design/embedded-agent-sdk-engine.md) §3 (seam), §4 (compatibility matrix), §5 (premises/hazards)
