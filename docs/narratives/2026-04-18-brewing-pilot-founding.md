...
- **Respect for the architectural principle**: the decision to hold the brewing script as "context packager" without LLM calls came out of the exchange with meta-Claude. If the script had called an LLM API directly, it would have broken agent-console's design identity of PTY orchestration with the platform staying out of the LLM's call path. Holding that line is the core of the differentiation.
