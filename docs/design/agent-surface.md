...
The enum has since grown past this initial spec: `restart` (Issue #1519), `setWorkerParameters` (Phase 3 above), and `decideMcpServerPermissions` (epic #1636 Phase 5 PR-3a, docs/design/embedded-agent-sdk-engine.md §4.5 D-C — `{ exposed: true, via: 'set_mcp_server_permission' }` on MCP, `{ exposed: false, ... }` on both UI (ships PR-3b) and embedded-visible (a *permanent* divergence from the MCP row, Architect-ruled: an embedded caller is refused inside the tool itself, never by table visibility)) each added their own table row without revising this table's initial snapshot — see each table's own source for current content.
...
### MCP reachability differs by agent kind

**MCP reachability differs by agent kind, and the difference is a property of the host, not of this server.** An embedded-agent worker reaches this instance's `/mcp` through the `init` command's `mcp.baseUrl` and a per-activation token, so an isolated instance's embedded agents always talk to that instance. A terminal-agent worker (Claude Code) reaches an MCP server through the host user's own registration (`claude mcp add`, stored in `~/.claude.json`, host-global or keyed by project path), which this server does not write; the PTY spawn injects `AGENT_CONSOLE_BASE_URL` and the identity variables, but Claude Code's tool discovery does not read them. Consequently a terminal-agent worker spawned by an isolated QA or dev instance calls whatever instance the host user registered, typically production, and its `delegate_to_worktree` / `send_session_message` calls land there. Measured during the 2026-09-09 pre-release QA (umbrella #1618, group A and group D reports): the isolated instance's own request log showed no MCP traffic from its terminal-agent workers while the registered instance received them.

Consequences the reader needs: (1) shipping-path verification of a terminal-agent MCP call must either register the isolated instance for the host user (`claude mcp add` scoped to the worktree path, removed afterwards) or drive the tool with a standalone MCP client against the isolated instance's `/mcp`, and the report must say which; a terminal agent "calling the tool" from inside an isolated instance is not evidence about that instance. (2) This is the coupling `docs/design/agent-cli.md` exists to remove (its "client config drift" motivation is this fact stated from the other side); until that design lands, the limitation stands and is recorded here rather than as a defect. No Issue is filed for it now: per-instance registration is a product decision (a per-worktree `.mcp.json` written by the server, or an env override Claude Code honours) and belongs to the agent-cli line, not to a fix.
