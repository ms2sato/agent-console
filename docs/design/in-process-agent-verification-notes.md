...
- **Restart-resume (conversation survives server restart): DEFERRED to post-v1 (fast-follow) — CONFIRMED by owner.** Rationale: under (b), a restart already kills + re-spawns the process exactly like today's PTY workers, so "no resume in v1" is parity, not a regression (resume is mandatory only under (a)). v1 ships the own-loop + structured-UI UX minimally; transcript persistence (with the hard mid-turn/mid-tool-call restore case) becomes an explicit fast-follow. Cost of deferring: worker-type behavior inconsistency (terminal continues via `-c`, new type resets) — must be stated in v1 doc/UI. Alternative if owner prefers UX consistency: include transcript persistence in v1.

## Next steps (ordered)

1. Both direction-changers are resolved (#878 in scope -> (b); restart-resume deferred). Write the **Design Decisions** section ((a)/(b)/(c) table) into `in-process-agent-system.md`: record **(b) as the chosen candidate**, #878-in-scope as the deciding factor, restart-resume as an explicit post-v1 fast-follow, and note the worker-type behavior inconsistency to surface in v1 doc/UI.
5. Re-review the revised doc with Fable (planned by owner) before advancing to a design PR.
