...
// Input
{
  ...
  fromSessionId?: string;    // Sender session ID. Optional for an embedded caller (defaults to the
                             // bearer token's session; a different value is refused -- Issue #1696);
                             // required for a terminal caller (AGENT_CONSOLE_SESSION_ID env var)
}
