...
| V5 health | `GET /api/config` (no auth), polled up to 10 x 1 s before deciding -- the first post-restart check that waits for the server to come up, so V6 after it reads a settled boot | HTTP 200 and `"authMode":"multi-user"` | any other status or a different `authMode` after the poll window | `curl` missing |
