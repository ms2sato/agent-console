...
**Implementation corrections to the table above (Issue (b), recorded here so the table and the code do not silently diverge; the table's cells are left as specified, this list is what the implementation actually does where it differs):**

- **V1's ExecStart comparison is derived from the template, not from a second copy of the unified path.** The template renders `ExecStart={{BUN_PATH}} run start` and `Environment=EMBEDDED_AGENT_BUN_PATH={{BUN_PATH}}` from the same placeholder, so "ExecStart = the template's unified bun" is implemented as "the live ExecStart path equals the live value of the Environment key rendered from ExecStart's placeholder". If a future template stops sharing the placeholder, the comparison is skipped (V3 still gates the binary). V1 also reports `LoadState=not-found` as CANNOT RUN (pointing at the setup script) rather than as "every key missing".
- **V3's interpreter is `<configured-bun>` itself**, not a literal `/usr/local/bin/bun`: `<elevate> runuser -u <User> -g <Group> -- <configured> scripts/lib/embedded-agent-bun-identity.ts <MainPID> <configured>`. On any unit that passed V1 the two are the same file (both render from `{{BUN_PATH}}`), and it keeps the unified path out of the lib. Two more CANNOT RUN markers exist beyond the table's: `MAINPID_0` (the unit has no main process) and `NO_MARKER` (elevation, `runuser`, the bun, or the entry failed before printing one); a bare `EMBEDDED_AGENT_BUN_PATH` short-circuits to `UNRESOLVABLE:bare` without elevating.
- **V4 has a CANNOT RUN case** (the table says `--`): `systemctl` absent, or present but producing no state (no systemd on the host). **V4's attached diagnostics and V6's `journalctl` read take the elevation prefix too** (the table lists elevation only for V2 and V3): the system journal is readable only by root / `adm` / `systemd-journal`, and the operator account is not assumed to be in either group -- the tier-3 `deployer` is not, and reads the journal as root in the driver.
- **V5 polls** `GET /api/config` up to 10 x 1 s before deciding: it is the first post-restart check that waits for the server to come up, so V6 after it reads a settled boot. The endpoint in the table (`/api/config`, no auth) is what shipped; the old `/api/auth/me` probe is gone.
- **V6's negative match is on the real warning texts** of `assessEmbeddedAgentBunPath`: a `"level":40` line carrying `EMBEDDED_AGENT_BUN_PATH` (four of the five warnings) or `bun-path identity check could not run` (the fifth, the server's own binary unresolvable, which does not carry the literal). `Server starting` must carry `"env":"production"` and `User mode initialized` must carry `"authMode":"multi-user"` on the same line, per the pino JSON shape `-o cat` prints.
- **The drift arm's mechanism is `sed` on the unit file + `daemon-reload`**, not a drop-in: a drop-in cannot unset ONE key visibly to `systemctl show -p Environment` (`UnsetEnvironment=` applies at exec time and leaves the property unchanged; an empty `Environment=` resets every key).
