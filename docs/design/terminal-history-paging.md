...
- `request-history.fromOffset` — interpreted as absolute. Serving rule: **forward catch-up serves the live window only.**
  ...
  - `fromOffset > totalAbsoluteOffset` (stale/diverged client): same recent-window response; with absolute offsets this can only happen across a worker restart (offsets reset to 0). Restart detection is handled authoritatively by the generation counter (Section 3.4); the `worker-restarted` app event (`poc-terminal-store.ts:349-356`) remains as a fast-path but is no longer load-bearing.
...
There is also a server-side accounting wrinkle in the same scenario: a pre-upgrade destructively-truncated file with no manifest means `liveBaseOffset = 0` **undercounts** the true cumulative stream by the destroyed head bytes, and revived activation then reseeds `worker.outputOffset` from that undercounted `getCurrentOffset` value, baking the undercount into the live series. On its own this is untidy but internally consistent (all post-upgrade series agree with each other); it is **subsumed by the generation counter** (Section 3.4): the first post-upgrade incarnation simply *defines* the epoch-0 offset baseline for that worker, and any client whose state disagrees resyncs on epoch mismatch instead of reasoning about offset magnitudes.

### 3.4 Worker generation (epoch) counter
...
This design therefore adds a **monotonic per-worker generation counter ("epoch")**:

- Bumped by 1 on every worker restart (the `restartWorker` / `resetWorkerOutput` path, Section 4.5); never reset or reused for the worker's lifetime.
- Persisted in the manifest (Section 4.2, `epoch` field) so it survives server restarts and hibernation. A missing manifest initializes it to 0 — the first post-upgrade incarnation is the epoch-0 baseline (Section 3.3).
- Carried as an **additive field** `epoch: number` on every `history`, `output`, and `history-range` server message (Section 5.1).
- Client behavior: the store records the epoch from the first epoch-bearing message it receives; on any mismatch between a subsequent message's epoch and the recorded one, it discards the live window and all paged state, resets `lastOffset` / `oldestOffset`, records the new epoch, and treats the payload as a fresh load — the same teardown the `worker-restarted` handler performs today (`poc-terminal-store.ts:349-356`), which is retained as a UX fast-path but is no longer load-bearing for correctness.
...
4. Write + rename the manifest with `pendingCutBytes = 0`. *(Crash here: safe — the manifest write is temp-then-rename, so the on-disk manifest is either still the step-2 version (`pendingCutBytes > 0`; recovery re-runs step 3, whose idempotence guard sees the sizes already match and falls through to redo step 4) or the completed version. No intermediate state is observable.)*
...
- **Worker restart**: `restartWorker` calls `resetWorkerOutput` (`worker-lifecycle-manager.ts:408`) and re-creates the worker with `outputOffset: 0` (revived: false, `:436-438`). `resetWorkerOutput` (`worker-output-file.ts:570-607`) must also delete all segments and rewrite the manifest with the **epoch bumped** (Section 3.4) so the absolute stream genuinely restarts at 0 under a new generation. This restart-triggered deletion acquires the **same per-worker promise-chain lock** as cuts and range reads (Section 4.3), making deletion atomic with respect to in-flight range reads: a range read either completes against the old incarnation's files or begins after they are gone — it never observes a half-deleted state. Client-side, the `worker-restarted` app event already resets the terminal and `lastOffset` (`poc-terminal-store.ts:349-356`); it must additionally clear the paged-in window (Section 6), and the epoch mismatch (Section 3.4) covers any client that misses the app-ws event.
...
- **Scroll anchoring**: on prepend (and on eviction), the view measures `scrollHeight` before/after the row-array change in a layout effect and adjusts `scrollTop` by the delta. The scroll container **must set `overflow-anchor: none`** as part of this design: the browser default (`overflow-anchor: auto`) runs its own anchoring heuristic in the same frame and would race the manual adjustment. Explicit compensation (rather than relying on CSS anchoring) is chosen because concurrent live appends at the bottom mutate `scrollHeight` in the same frame and the store cannot rely on browser anchoring heuristics across both edges. Effect ordering: the existing bottom-follow layout effect (`PocTerminalView.tsx:222-236`) and the new prepend-anchor effect have mutually exclusive firing conditions by construction (pinned-to-bottom vs. top-edge prepend), but the prepend-anchor effect must run first, and both effects must guard against firing together in the degenerate case where the content is shorter than the viewport (the view is simultaneously "at top" and "at bottom").
...
- Epoch: initializes to 0 with no manifest; persisted across manifest rewrites; `resetWorkerOutput` bumps it by exactly 1 and it stays monotone across repeated resets; carried on `history` / `history-range` responses.
- `resetWorkerOutput` clears segments and rewrites the manifest (epoch bumped); `deleteWorkerOutput` removes segments + manifest; `deleteSessionOutputs` leaves nothing.
...
- Epoch mismatch on any of `output` / `history` / `history-range` clears live + paged state, resets offsets, records the new epoch, and treats the payload as a fresh load — including the aliasing fixture: a stale `beforeOffset` that is numerically valid in the new epoch must trigger a reset, never a prepend of aliased data.

**Integration (wire)** (server WS route tests alongside existing `routes` tests): `request-history-range` over a real upgrade path; range fetch racing live `output` (assert offset coherence); two connections paging the same worker; hibernated worker restore + immediate range request; worker restart between two range requests (assert the next response carries the bumped epoch); timeout path returns the error shape.
