...
1. Compute the slice point exactly as today: `currentSize - floor(fileMaxSize * 0.8)`, advanced to a UTF-8 character boundary by skipping continuation bytes -- **and then on to the start of the next line** ([#1445](https://github.com/ms2sato/agent-console/issues/1445)). A character boundary is not a record boundary, and the live file is NDJSON: a cut landing mid-line hands the restore path a partial first record, which it reads from the base of the window. Fall back to the character boundary when the remainder holds no newline, or when the only one is its final byte -- honouring that would archive the whole file and leave an empty live window, which the restore side cannot recover, unlike a leading fragment, which it can. At defaults this yields ~2MB uncompressed per segment.
...
- Construct with the live terminal's current `cols` (wrap parity with the visible window), the live terminal's current `rows` (geometry parity: TUIs author cursor-positioning/erase sequences against the real terminal height, and replaying them into a differently-sized screen misplaces bottom-chrome redraws — status bars, input frames — which then scroll into the throwaway buffer as spurious "settled" rows; issue #979). Unlike `cols`, a rows-only mismatch cannot corrupt extracted row geometry (wrap width), it only degrades the TUI-churn approximation, so paged chunks are NOT dropped on a rows-only resize — a chunk replayed at slightly-stale rows is an acceptable approximation. Use `scrollback: 100_000`. This is a fixed generous bound, not a "typical case" estimate: the true worst case for a 256KB chunk is one row per byte (newline-heavy output → 262,144 rows), which no comfortable scrollback covers. **Overflow degradation loop**: after replay, if the throwaway buffer's length has reached the scrollback cap, rows were silently dropped from the top and the replay is not authoritative — discard it and re-request the same range with `maxBytes` quartered (256KB → 64KB → 16KB, floor 16KB). At 16KB even the one-byte-per-row worst case (16,384 rows) fits within the 100k scrollback, so the loop structurally terminates within two degradation steps.
...
**Seam correction — older-neighbor pair re-replay (#979)**: a range chunk begins at an arbitrary VT midpoint, so its replay starts without the terminal state the preceding bytes established. For relative-repaint TUIs (Claude Code emits only CUU/CUD + EL — no absolute positioning, no scroll regions) the chunk's leading frame-tail clamps at the top of the empty throwaway screen, prints as ordinary content, and later scrolling settles it into scrollback: roughly one chrome artifact per fetch seam. Replay height cannot help (relative movement is height-independent; measured identical at 24 vs live rows on the motivating capture). The correction:

- **Raw retention (top chunk only)**: the store retains the RAW bytes of the current TOP (oldest) paged chunk — and only that chunk — alongside its rows. Bound: one chunk ≤ the range request's `maxBytes` (256KB), so retained raw never exceeds one request budget. The raw is dropped whenever the chunk itself goes away (eviction, cols-drop, resync, teardown) and is REPLACED when a new top arrives.
- **Pair re-replay on fetch**: when the next older chunk `C_new` arrives (stream position: `C_new` precedes the current top `C_top`), replay `C_new.bytes + C_top.bytes` in one throwaway terminal. Record `splitBaseY = buffer.baseY` after `C_new`'s bytes finish parsing (the scrollback rows settled so far are `C_new`'s; the still-volatile screen rows are exactly the state `C_top`'s leading repaints consume — the seam correction itself). Extract rows once at the end and partition at `splitBaseY`: the first `splitBaseY` rows REPLACE `C_new`'s standalone extraction (they are its settled rows), the remainder REPLACES `C_top`'s rows (now seam-free). The partition guarantees no duplicated and no lost rows at the boundary. Both replaced chunks get fresh negative keys (the §6.3 allocator never reuses keys); the prepend-anchor arithmetic already compensates by the net paged-row-count delta, and the replacement happens at the very top of the window, above the viewport, so the §6.3 stability argument is unchanged.
- **Fallback**: if the pair replay overflows the throwaway scrollback (the overflow rule is unchanged), fall back to the standalone replay of `C_new` for this fetch — that one interior seam stays uncorrected rather than degrading the fetch loop.
- **Accepted residual**: the CURRENT top chunk's own leading seam has no older neighbor until one is fetched; at the retention floor or offset 0 none will ever arrive. Up to a few artifact rows at the very top of the paged window are accepted (and sit directly under the Section 6.1/#980 notices); interior seams are artifact-free by construction.
- **Side benefit**: link detection (Section 6.3) runs over the pair replay's joined extraction, so wrapped URLs crossing the corrected chunk boundary now join — narrowing the per-chunk-isolation caveat noted below.
