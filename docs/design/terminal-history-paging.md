...
4. On worker restart the server closes all open worker WebSockets (normal closure) so the reconnect delivers the new epoch; the app-ws `worker-restarted` event is a UX fast-path only (Sections 3.4, 4.5).
