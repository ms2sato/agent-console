...
- **Storage** — `repository_orchestrator_sessions (repository_id, session_id, created_at)`, `PRIMARY KEY (repository_id, session_id)`, `ON DELETE CASCADE` on both foreign keys (a deleted session's designation disappears; a deleted repository's designations disappear). The v40 column `repositories.orchestrator_session_id` was backfilled into the table then set to NULL by v41, and dropped in v42 (Issue #1725) — it carried a `REFERENCES` constraint, which SQLite's `DROP COLUMN` refuses, so the drop required a table rebuild. On the wire, `Repository.orchestratorSessionIds: string[]` (sorted by designation time, then id).
- **Restart survival** — a designation survives a server restart, because the boot path (`SessionInitializationService.initialize()`) upserts the session set rather than deleting and re-inserting it (Issue [#1762](https://github.com/ms2sato/agent-console/issues/1762)); a `-c` resumed PTY Orchestrator therefore does not need to re-run its First Action for routing to continue.
...
2. **Visual distinction for shared sessions** — badge, colour, or label so users know they are typing into a shared space (reduces accidental secret leakage into a visible-to-all terminal). The client learns whether a session is shared via a new boolean on the session payload (provisional name `isShared`), derived server-side from `sessions.created_by` resolving to a configured shared account. Storing it as a derived field rather than a stored column keeps the source of truth in `users` / configuration; the alternative — exposing `created_by` and the shared-account user-id list to the client — is rejected to avoid leaking internal identity layout. The same derivation pattern is reused for `Session.createdByUsername` (the human-readable session owner / worktree owner shown in the active-sessions sidebar in multi-user mode, Issue [#914](https://github.com/ms2sato/agent-console/issues/914)): resolved server-side via `UserRepository.findById(created_by)?.username`, populated on the wire as `string | null`, never persisted.
