...
export interface EmbeddedAgentDefinition {
  ...
  instructions?: string[];    // opt-in instruction-file list, resolved relative to locationPath via resolveConfinedPath — see Instruction loader
...
type EmbeddedAgentCommand =
  | { v: 1; type: 'init';
      ...
      instructions?: string[];                // opt-in instruction-file list, forwarded unchanged; the loop resolves + confines + loads them — see Instruction loader
...
**Extra confinement roots for `Read` only (#1570).** `resolveConfinedPath` takes an optional third parameter, `extraRoots: string[]`, checked with the same symlink-following rule as `locationPath` (step 3 above becomes "confined iff prefixed by `locationPath` OR by one of `extraRoots`"). `init.context.attachmentRoots` (see [Stdio protocol](#stdio-protocol-v1)) is the only source of extra roots today, and only `read.ts` forwards it — `write.ts` / `edit.ts` / `glob.ts` / `grep.ts` still call `resolveConfinedPath` with no third argument, so they continue to reject any path under it. This is why message attachments (uploaded to a directory outside `locationPath`, keyed by the server process's uid: one directory per server, group-readable across all users in multi-user mode, unguessable per-upload names — the same boundary the PTY branch already delivers paths into) are readable by `openai-api`'s builtin `Read` tool but not writable/editable/searchable by the other builtin tools. A root that does not exist on disk yet (no upload has ever happened for this server) is dropped rather than raising, so it confines nothing rather than crashing the check. `claude-sdk` does not consume `attachmentRoots` at all — see the field's own doc comment on `EmbeddedAgentInitCommandBase.context` for why its native `Read` tool needs no such extension.
...
**Confinement and default-off, same as prior fast-follows.** Both tools resolve their target through the same `resolveConfinedPath` helper as [`Read` / `Glob` / `Grep`](#path-confinement-the-ff-1a-minimum-floor) — no new confinement mechanism. Like `Bash`, both are mutating tools and stay OFF by default: `DEFAULT_EMBEDDED_AGENT_ENABLED_TOOLS` does not include them (`['Read', 'Glob', 'Grep', 'TodoWrite']` as of #1573 below), so a definition must opt in explicitly via `enabledTools` to expose `Write` and/or `Edit` to the model.

### `TodoWrite` (#1573)

| Tool | Args | Behavior |
|---|---|---|
| `TodoWrite` | `{ todos: { content: string; status: 'pending' \| 'in_progress' \| 'completed'; activeForm: string }[] }` | Replaces the agent's entire task list on every call (full replace, not merge — an empty `todos: []` legitimately clears it to zero items). No filesystem or process access; purely a planning/progress signal to the user. |

**One input shape, two independent implementations, MCP-served on `claude-sdk` (Issue #1575).** `TodoWrite` was originally designed on the premise that it is the Claude Agent SDK's own native builtin on that engine — measured false (Issue #1573/#1575, [§4.1](embedded-agent-sdk-engine.md#41-tool-surface--the-inverted-parity-question-ruled) / [§5.2](embedded-agent-sdk-engine.md#52-the-type-surface-and-the-resolved-cli-catalog-can-disagree-issue-1573-2026-09-03) of embedded-agent-sdk-engine.md): the resolved CLI silently drops an unrecognized native `TodoWrite` from `system:init`'s reported tool catalog. `sdk-engine.ts` instead serves it via an **in-process SDK MCP server**, on the SAME `console` server that already serves [the `Compact` tool](#the-compact-tool) — `createSdkTodoWriteTool()`, registered (both the server tool and the `mcp__console__TodoWrite` allowlist entry) only when `TodoWrite` is present in the definition's `enabledTools`, unlike `Compact`'s unconditional registration. On `openai-api`, it is a builtin (`packages/embedded-agent/src/tools/todo-write.ts`), whose input is validated with **valibot** rather than the hand-rolled `parseArgs` pattern the other builtins use — deliberately pinned against a schema matching the SDK's own declared `TodoWriteInput` shape (`sdk-tools.d.ts`) field-for-field, so both engines' models see (and both engines' `tool-call` events carry) the exact same argument shape. `sdk-engine.ts` imports this same schema and its `summarize()` result-text helper rather than re-declaring them, so a malformed call is rejected with identical wording on both engines. That shape-parity is what lets the client render both engines through **one** component (`TodoPanel`, below) with no per-engine branch — the client accepts either the plain `'TodoWrite'` name (openai-api) or the namespaced `mcp__console__TodoWrite` name (claude-sdk).

**Per-incarnation state, not a filesystem or server write.** The `openai-api` implementation holds "the last accepted list" as private closure state inside `createTodoWriteTool()` — a factory `packages/embedded-agent/src/tools/index.ts`'s `resolveEnabledBuiltinTools` calls fresh on every invocation, unlike the other six builtin tools (which are shared, stateless module-level singletons). This is what makes a second incarnation start with an empty list rather than inheriting the previous one's: each subprocess incarnation gets its own instance. No new persistence — the durable record of the task list is the transcript itself (the `tool-call` entries), not this in-memory state, which exists only to compose the `execute()` result string's item counts within a single incarnation. `createSdkTodoWriteTool()` in `sdk-engine.ts` mirrors this same per-incarnation-closure shape for `claude-sdk`.

**Coexistence with a future native catalog entry.** `handleSystemInit`'s existing `reportedNonMcp` warn already logs the raw reported tool catalog whenever a session requests `TodoWrite` — should a future SDK version start reporting it natively, that warn surfaces the change with no new code needed; removing the MCP-served registration in favor of the native path is deferred to that SDK-bump PR.

**On by default.** Unlike `Bash`/`Write`/`Edit`, `TodoWrite` ships ON in `DEFAULT_EMBEDDED_AGENT_ENABLED_TOOLS` — it writes no files and has no side effects outside the transcript, the same rationale as `Read`/`Glob`/`Grep`. In the Add/Edit form (`EmbeddedAgentForm.tsx`), it is grouped under "Read-only" alongside them.
...
4. Reset the output stream — mint a new epoch and truncate, exactly like `restartAgentWorker` does via `resetWorkerOutput` (`worker-manager.ts:411-433`, `worker-output-file.ts:1067-1108`). **Every activation is restart-semantics in v1** (fresh conversation); there is no revive path (contrast `activateAgentWorkerPty`'s `revived: true` epoch-preserving branch, `worker-manager.ts:322-338` — deliberately not used, per the restart-resume deferral). **Superseded at this step by Transcript Restore** ([below](#transcript-restore)): every activation now attempts reconstruction in place of this plain reset when the persisted stream has a boundary to reconstruct from, so "there is no revive path" describes only the case Restore's own reconstruction cannot cover.
...
- **`restore-info` (Transcript Restore, #1123; `sdkResumed` added by R1/#1410; failure form added by #1449).** A `WorkerServerMessage` variant, now a **discriminated union of two forms** distinguished by `failed`: the success form `{ type: 'restore-info'; epoch: number; restoredMessageCount: number; repairedToolCallIds: string[]; completed: boolean; sdkResumed?: boolean; failed?: false }`, and the failure form `{ type: 'restore-info'; epoch: number; failed: true; sdkResumed?: boolean }`. `sdkResumed` is set **only** by the `claude-sdk` engine on EITHER form (`openai-api` omits it on both; absence means "this engine has no such concept" and must never be read as `false` — the client tests `=== false` explicitly), added to `packages/shared/src/types/session.ts`'s `WorkerServerMessage` union (and `WORKER_SERVER_MESSAGE_TYPES`). Sent on BOTH an activation's restore success and its restore FAILURE (never on a first-ever activation, where there is nothing to declare). Dual delivery (fast-path push to currently-attached connections right after the restore attempt -- success or failure -- before spawn; bootstrap re-delivery to every new connection for the lifetime of the incarnation) applies to BOTH forms; the third, `ready`-triggered completion repush (re-sending with `completed` flipped to `true`) applies to the SUCCESS form only, since the failure form has no `completed` field -- see [Transcript Restore § UI](#transcript-restore) for the full mechanism and rationale (`completed`'s corrected role is described there, Issue #1205; the failure form's D1/D2/Loss rationale is described there too, pointing at [#1447](https://github.com/ms2sato/agent-console/issues/1447) and [#1449](https://github.com/ms2sato/agent-console/issues/1449) rather than restating it here).
...
`create_timer` and `create_conditional_wakeup` now guard with `canReceiveNotifications` instead of `isPtyBackedWorker`. `isPtyBackedWorker` itself is unchanged and, after this PR, has two remaining production consumers outside its own definition: the PTY-vs-embedded branch inside the new delivery seam below, and `send_session_message`'s explicit-target guard in `mcp-server.ts` (which combines it with `canReceiveSessionMessages` so terminal workers stay valid explicit targets) (`run_process`'s guard was PR B's pending migration — see the Update below, now done).

The seam, `SessionManager.deliverWorkerNotification(sessionId, workerId, params, opts?)`, is now the single place that turns a structured notification (`PtyNotificationParams`, the same shape `writePtyNotification` already consumed) into either a PTY write (`isPtyBackedWorker(worker)` true) or an `EmbeddedAgentWorkerService.sendSystemNotification` call (`worker.type === 'embedded-agent'`), returning `{ ok: true } | { ok: false; error }`. Three callers were migrated onto it in this PR: `app-context.ts`'s timer and conditional-wakeup callbacks (previously composing `writePtyNotification` directly against a PTY-only assumption), and `send_session_message`'s two delivery branches in `mcp-server.ts` (previously a hand-rolled `if (worker.type === 'embedded-agent') {...} else {...}` pair that pre-dated the seam and is now one of the seam's callers instead of a parallel implementation). The interactive-process EXIT notification (`app-context.ts`, `internal-process` kind) and `process-output-router.ts`'s stdout-routing notification remained PR B's pending migration (`run_process` support) — see the Update below, now done.

A mid-turn embedded-agent worker (a turn already in flight when a notification arrives) no longer rejects the notification outright: `EmbeddedAgentWorkerService` gained a per-worker FIFO queue (`Runtime.pendingNotifications`, capped at 32 entries with oldest-drop-on-overflow) so `sendSystemNotification` parks the notification instead of returning `TURN_IN_PROGRESS`, and delivers it as the next ordinary turn on the worker's next `state: idle` event — one queued entry per idle event, in arrival order. This applies ONLY to system notifications (timers, conditional wakeups, inter-session messages routed to an embedded target, and — since PR B — `run_process` chunks); a human's own `sendUserMessage` mid-turn is unaffected and still rejects with `TURN_IN_PROGRESS` exactly as before. See [Mid-turn notification queue](../glossary.md#mid-turn-notification-queue) in the glossary for the concept's canonical definition.

**Update (Issue #1574 PR B): `run_process` (both `outputMode`s) and the interactive-process exit notification joined the seam, completing the migration above.** `run_process`'s guard moved from `isPtyBackedWorker` to `canReceiveNotifications`, matching `create_timer`/`create_conditional_wakeup`'s error text. The two remaining direct `writePtyNotification` call sites this Issue's scope covered are now gone: `process-output-router.ts`'s `ProcessOutputRouterDeps.writeInput` dep was replaced with a `deliverNotification` dep bound to the seam (both `outputMode: 'pty'`'s full-content notification and `outputMode: 'message'`'s brief per-chunk notification go through it, fixture-pinned byte-identical to the pre-PR composed params), and `app-context.ts`'s interactive-process EXIT callback (a fourth producer this Issue's scope did not originally enumerate, found while reading the code for this PR) was migrated the same way as the timer/wakeup callbacks above. `writePtyNotification` now has exactly two direct callers outside its own definition and the seam's own PTY-backed branch: `inbound/handlers.ts` and `routes/review-queue.ts`, both hard-scoped to `agent`-type workers for unrelated notification kinds and permanently out of this Issue's scope (pinned as an exact-set assertion in `pty-notification-delivery-containment.test.ts`).

**Further update (Issue #1591):** stdout/response content routing and the exit notification for the same process now share a single per-process delivery tail in `process-output-router.ts` (`routeProcessContent` enqueues content, `routeProcessExit` enqueues the exit notification behind it), so the exit notification always delivers after any still-in-flight content notification for that process instead of racing ahead of it.

`run_process`'s `outputMode: 'message'` writes stdout to a file under the session's messages directory (`SessionDataPathResolver.getMessagesDir()`), which sits outside the session's `locationPath` — an embedded-agent worker's `Read` tool needs to reach it. Measured on both engines before deciding whether a fix was needed: the `claude-sdk` engine's native tool set already reaches it (that engine bypasses the `attachmentRoots`-confined builtin `Read` entirely, per this doc's SDK-engine sections), so no change; the `openai-api` engine's confined `Read` rejected it with `"Access outside session location is not permitted."` until `EmbeddedAgentWorkerService`'s `attachmentRoots` composition (already carrying the upload dir since #1570) was extended to also include `resolver.getMessagesDir()`.
...
Tool definitions come from the MCP client's `listTools()` at init: MCP already publishes JSON-Schema `inputSchema` per tool, mapped 1:1 onto the OpenAI `parameters` field. The system prompt is assembled by the loop at init, in this order: (1) context preamble (session id, worker id, cwd — so the model passes correct identity arguments); (2) discovered/opt-in **instruction segments**, then the **rules layer** (see "Instruction loader" below); (3) `EmbeddedAgentDefinition.systemPrompt` — operator configuration comes last so it wins on conflict.

### Instruction loader

**Since Issue #1343 Phase A**, both engines call the SAME `loadInstructions` (`packages/embedded-agent/src/system-prompt.ts`) — the `claude-sdk` engine no longer has a narrower opt-in-only path (its `settingSources: []` pin, which disables the SDK's own NATIVE settings-derived discovery, is unaffected; this loader is what delivers the equivalent content from outside that mechanism, for both engines — see [embedded-agent-sdk-engine.md §4](embedded-agent-sdk-engine.md)). The loop discovers instruction files and rules across four layers at every activation and injects their content into the system prompt, immediately after the context preamble and before `EmbeddedAgentDefinition.systemPrompt`:
...
3. **`instructions[]`** — an opt-in, per-definition explicit file list (`EmbeddedAgentDefinition.instructions?: string[]`, opencode-shaped). Each entry is resolved relative to the session's `locationPath` (the same root builtin tools use) through `resolveConfinedPath` (`packages/embedded-agent/src/tools/path-confinement.ts`) — the identical confinement helper builtin tools use, so an `instructions[]` entry can never read outside the session's working tree even when the defining `EmbeddedAgentDefinition` was authored by a different party than the executing user. Escape attempts (absolute paths outside confinement, symlink escape via `realpath`) are skipped and warn-logged, never fatal. **Deduped by resolved path** against the global/chain layers above: an entry that names a file already discovered there (e.g. a definition that redundantly lists `CLAUDE.md`) is dropped rather than appended a second time.
4. **Rules** — every `<gitRoot>/.claude/rules/*.md`, sorted by file name. A rule's `paths:`/`globs:` frontmatter (either spelling; string or array; the multi-line YAML list this repo's own rules use) determines its scope: **unscoped rules** (no such frontmatter) are included eagerly, the same way an instruction segment is, under a `--- Rule: <origin> ---` delimiter. **Scoped rules** are NOT included at session start — the index only lists their name and globs in a one-line summary ("Rules that apply when you touch matching paths: ..."), so the model knows they exist without paying their content cost up front. No git root, or no `.claude/rules` directory, produces an empty layer silently — both are routine.

   **Lazy activation (Issue [#1343](https://github.com/ms2sato/agent-console/issues/1343) Phase B).** The first time a builtin tool call's path argument matches a scoped rule's globs, that rule's full content is activated — delivered to the model — instead of staying an unreachable index entry. One class, `RuleActivator` (`packages/embedded-agent/src/rule-activation.ts`), implements this identically for both engines: a pure `matchScopedRules(toolName, args)` query decides which not-yet-activated rules a call touches, and an effectful `activate(names)` command reads each matched rule's file (fresh, at activation time — not cached from index time) and returns a formatted block, or `null` when nothing new was activated.

   **Match rule.** The candidate path argument is resolved by tool name, not by sniffing every argument for something path-shaped: `Read` reads `path` (the `openai-api` engine's own builtin) or `file_path` (the native `claude` CLI's builtin, consulted in that order so one shared table serves both engines); `Write`/`Edit` read `file_path`; `NotebookEdit` (SDK arm only) reads `notebook_path`; `Glob`/`Grep` read the optional `path` argument as a DIRECTORY candidate when present, and produce no candidate at all when absent (never falling back to `cwd`). `Bash`, `TodoWrite`, `Compact`, and every MCP tool name never produce a candidate, by construction — the lookup is keyed on tool name first, so an argument that happens to look like a path never matters for a name outside this table. A candidate is resolved against `cwd`, then expressed relative to the git root; a candidate outside the git root matches nothing. A file candidate matches when `Bun.Glob` matches the rule's glob against the relative path; a directory candidate (Glob/Grep) matches when the glob matches a synthetic `<dir>/x` — "the rule would apply to something under that directory," not that the directory itself is a file.

   **Delivery differs by engine, because "beside the tool result" means something different in each.** On `openai-api`, `CompositeToolExecutor` sets `ToolCallOutcome.appendix` to the activation block; `AgentLoop` composes `truncate(result) + '\n\n' + appendix` for BOTH the persisted `tool-result` event and the provider-facing tool message — truncation is applied to the tool's own output FIRST, appendix concatenated AFTER, so the 16 KiB `TOOL_RESULT_MAX_BYTES` cap (see "Synthetic tool-call ids" below for another per-turn normalization at this same layer) never cuts a rule file mid-content; a rule up to the shared rules-layer budget survives whole. On `claude-sdk`, tools run natively inside the CLI, so there is no tool result to append to from outside — a `PostToolUse` hook (registered in `sdk-engine.ts`'s `buildOptions`, no `matcher` field; tool-name filtering happens inside the callback via `RuleActivator.matchScopedRules` itself, the same way the pre-existing matcher-less `PostCompact` hook filters by `hook_event_name`) returns the identical block as `additionalContext`, which the SDK delivers to the model directly. **This hook mechanism is empirically verified, not assumed** — see [embedded-agent-sdk-engine.md §4.2](embedded-agent-sdk-engine.md#42-instruction-loader--corrected-coderabbit-finding-sdk-engine-phase-1-implementation-pr) for the Task 0 probe that confirmed `additionalContext` reaches the model on this build, at sizes well past this repo's own budget.

   **Budget.** Lazy activations draw from the SAME `RULES_LAYER_CAP_BYTES` pool the eager unscoped layer already spent from — there is no separate budget for lazy activation, and (per the Task 0 measurement above) no separate size threshold for the `claude-sdk` hook either. A rule that does not fit in the remaining budget is not activated; the block carries a declared `[rule not activated for size: <name>]` line instead, once, in the same style as the eager layer's "rules omitted for size" line — the omission is visible in-band, never silent.

   **Once per incarnation, and restore does not double it — except on one engine, by design.** Each `RuleActivator` instance tracks which rule names it has already activated and never re-activates one. On `openai-api`, [Transcript Restore](#transcript-restore) reconstructs the conversation from the persisted stream; `reconstructConversation` also collects `activatedRuleNames` from every restored `tool-result` event's own structured `activatedRules` field — never parsed out of `result`'s text, so a restored tool output that coincidentally or maliciously contains the literal `[rule activated: <name>]` substring has zero effect — and forwards it to the subprocess on the `init` command, where `main.ts` seeds the new incarnation's activated set from it, so the SAME rule is never delivered twice across a restart. On `claude-sdk`, resume works differently: the SDK resumes its own session state directly rather than being handed a reconstruction, so a fresh `RuleActivator` with an EMPTY activated set is constructed on every incarnation, including a resume — a resumed session may therefore receive a rule's content a second time. This is an accepted, cost-only duplication (the rule's own content is idempotent to re-read; nothing breaks), not worked around by persisting the set separately, because that would make this loader a second, competing writer of state the SDK already owns.
...
**Concatenation and delimiters.** Segments are joined in the order: global -> chain (root to cwd) -> `instructions[]` (array order, deduped) -> rules layer (unscoped rule segments, then the omission line if any, then the scoped-rules index line if any) -> `definition.systemPrompt`. Each instruction segment is preceded by a one-line delimiter: `--- Instructions: <origin> ---`; each unscoped rule segment by `--- Rule: <origin> ---` — both `<origin>` are the absolute resolved source path.

**Caps and overflow — two independent budgets.** The instructions layers (global/chain/`instructions[]`) keep their pre-existing budget: each file capped at 16 KiB, the aggregate of all such segments (excluding `definition.systemPrompt`) capped at 48 KiB (`INSTRUCTION_PER_FILE_CAP_BYTES` / `INSTRUCTION_AGGREGATE_CAP_BYTES`). Overflow drops whole segments from the general side first — global, then chain starting from the root end, then `instructions[]` starting from the last array entry — until the total is back under the cap. The rules layer has its OWN, separate budget, `RULES_LAYER_CAP_BYTES` (default 160 KiB, env-overridable — sized for this repo's own unscoped-rules total, 121 KB as of 2026-09-03, plus headroom): unscoped rules are **never truncated mid-file** (a half rule is worse than none), so overflow drops WHOLE files, largest-first, until the total fits, and appends a one-line declared omission ("rules omitted for size: <names>") so the loss is visible in-band rather than silent. Neither budget affects the other. The only other operator-visible signal for either budget is a warn-level log line (origin + byte size) for every per-file truncation and every drop. **The `claude-sdk` engine now carries this same rules-layer weight, since it composes `systemPrompt.append` from the identical `loadInstructions` result**: up to `RULES_LAYER_CAP_BYTES` (160 KiB default, roughly 40K tokens at a ~4-bytes-per-token estimate) can land in `systemPrompt.append` — this repo's own unscoped-rules footprint is 121 KB of that ceiling. It is paid once per incarnation (activation), not per turn, matching how the instructions layers already worked before this change.

The loop already runs as the requesting OS user, so filesystem permissions are naturally correct in multi-user mode — no new elevation surface. All instruction/rule files are read once at activation and cached for the worker's lifetime; a restart re-reads (consistent with restart semantics), but there is no first-tool-call deferral and no filesystem watcher for the eager (unscoped) content, since it is structurally part of the system prompt and cannot be deferred past the first provider request. (Phase B's scoped-rule activation is deferred by design — that is its whole point.)
...
**Synthetic tool-call ids.** Some `openai-api` providers (observed: `opencode-go` / `qwen3.8-flash`) return an EMPTY `callId` (`""`) for one or more tool calls in a turn; two empty ids in the same iteration are indistinguishable to any consumer keyed by `callId`, and because ids are persisted, the ambiguity would otherwise survive into restore. `AgentLoop.runUserTurn` normalizes this once per provider iteration — immediately after the provider outcome is obtained and before any consumer (the assistant-message builder, the wire `tool-call`/`tool-result` emitters, `fillPendingToolResponses`) reads `outcome.toolCalls` — via `assignSyntheticToolCallIds` (`tool-call-ids.ts`): an empty `callId` is replaced with `` `synthetic:<turnId>:<iteration>:<index>` ``, while a provider-supplied non-empty id passes through untouched. The scheme is deterministic (a restore replay recomputes the exact same id from the same persisted `turnId`/iteration/index, so the reconstructed transcript is never a different conversation than the one that was actually lived) and unique within a worker (`turnId` is unique per turn, `iteration` unique per turn, `index` unique per iteration).
...
- **File attachments (#1570).** The composer (shared `MessagePanel`) supports attaching files to a message sent to an embedded-agent worker, the same UI affordance as a PTY-worker send. The wire-level handling differs: embedded delivery has no first-class file-attachment concept, so `SessionManager.sendMessage`'s embedded-agent branch folds the saved upload paths into the delivered text as a labelled `Attached files:` block (`composeEmbeddedAgentDeliveryText`) rather than passing them as typed lines the way the PTY branch does. `openai-api`'s builtin `Read` tool can then open an attached file directly (see [Path confinement](#path-confinement-the-ff-1a-minimum-floor)'s `attachmentRoots` extension); `claude-sdk` needs no such extension, since its native `Read` tool already reaches outside `cwd` under our production `permissionMode`.
- A persistent, non-dismissable note in the view: conversation resets when the worker or server restarts (the v1 worker-type inconsistency called out in [Design Decisions](#design-decisions)). **Since Transcript Restore (#1123)** this note's exact wording/condition depends on the worker's [Engine](../glossary.md#engine-embedded-agent--openai-api--sdk): `openai-api` workers keep the "restored automatically" wording (accurate — their conversation IS reconstructed); a `claude-sdk`-engine worker normally gets **no** notice, because R1 ([#1410](https://github.com/ms2sato/agent-console/issues/1410)) resumes the SDK session and the conversation genuinely does continue; the distinct notice Phase 3 ([#1335](https://github.com/ms2sato/agent-console/issues/1335)) shipped survives as the FALLBACK, shown whenever **this incarnation's SDK session did not resume** (`restore-info.sdkResumed === false`) — the outcome, not an attempt, since three of its four routes never send a `resume` at all — see [embedded-agent-sdk-engine.md §4.3](embedded-agent-sdk-engine.md#43-ui-the-restore-divergence-notice-sdk-engine-phase-3-polarity-inverted-by-r1) for the inverted polarity table and why porting the old unconditional rule forward would show a permanent false warning.
- `exited` events render as a plain historical transcript row (no affordance); a separate Restart action, driven by the worker's CURRENT exit state rather than by which row is last in the transcript, renders as a sibling element below the transcript while that state holds (re-activation = fresh conversation). See the `reason: 'evicted'` subsection below for the evicted-row's own no-affordance rendering, and R1 (#1455) for why the affordance moved out of the transcript. **Since #1454**, a row whose `stderrTail` is present (unexpected exits only) additionally renders a native `<details>` disclosure, "Show process output", containing the tail verbatim -- the row itself stays a fact; the disclosure adds no new affordance beyond revealing what the process already said.
...
- **`TodoPanel` (#1573).** A chrome row mounted as a sibling immediately above `MessagePanel`, next to the compaction chrome. It carries **no new wire event and no new store field** — it is DERIVED, in the component, from `entries`: the latest `tool-call` entry named either `TodoWrite` (`openai-api`) or `mcp__console__TodoWrite` (`claude-sdk`'s MCP-served name, Issue #1575) whose `result.ok === true` supplies the current list (`args.todos`); a later `TodoWrite` call whose result is `ok: false` does not replace the currently-shown list. Because this is pure derivation over the same `entries` array every other row already reads, restore, history replay, and reconnect all get it "for free" — there is nothing engine-specific or transport-specific to wire up. Hidden entirely when no `TodoWrite` call exists in the transcript; collapsed by default (via `<details open={!allCompleted}>`) once every item is `completed`, with a persistent one-line summary (`N/M completed`) whether collapsed or open. Each item shows a status glyph plus, for `status: 'in_progress'`, its `activeForm` text in place of `content`. Read-only — no editing affordance (see `TodoWrite`'s Non-goals). The per-call `ToolCallCard` row in the ordinary transcript is unaffected and keeps rendering every `TodoWrite` call as history, same as any other tool call — `TodoPanel` is an additional, derived summary, not a replacement for that row.
...
**Status:** implementation-grade spec (Issue [#1572](https://github.com/ms2sato/agent-console/issues/1572)).

**The problem this closes.** Before this, the composer's slash-completion affordance (`slashCompletionEnabled` on `MessagePanel`) was hardcoded `false` for both embedded-agent engines — not because completion was unimplemented, but because turning it on with no backing data would have offered commands the underlying engine does not honour, silently sending them as ordinary prose to the model. A user typing `/compact` on `openai-api` (which has no slash-command parser of its own) would have the literal text `/compact` become the next thing the model reads.

**Single writer.** `EMBEDDED_AGENT_SLASH_COMMANDS` (`packages/shared/src/types/embedded-agent-slash-commands.ts`) is the SINGLE WRITER of which slash commands exist per engine and how each is handled. Both the client's completion source and the server's interception (below) read this table; neither re-derives it. Its own doc comment is the canonical text for per-entry content — this section states the contract, not the content, to avoid the two drifting (see `.claude/rules/design-principles.md` on sweeping for what a change invalidated).

**Two handling modes, named on the table entry itself (`handledBy`):**

- **`'engine'`** — the command is forwarded to the subprocess as an ordinary `user-message`, exactly like any other text the user typed. The engine itself recognizes and interprets it. This is `claude-sdk`'s mode for its own built-in slash commands (`/compact`, `/cost`, `/context`) — the Claude Agent SDK already parses these out of the streamed prompt.
- **`'console'`** — the SERVER intercepts the command before it ever reaches the engine as prose, and maps it to an existing console operation instead. This is `openai-api`'s mode for `/compact`: that engine has no slash-command parser of its own, so the interception IS the feature. See [`EmbeddedAgentWorkerService`](#server-side-management-embeddedagentworkerservice)'s `sendUserMessage` / `resolveConsoleSlashCommand` / `deliverUserTurn`'s `commandOverride` parameter — the WIRE command sent to stdin differs (`{v:1, type:'compact'}` instead of `{v:1, type:'user-message', ...}`), while the PERSISTED transcript event is unchanged: it always shows exactly what the user typed, matching how an `engine`-handled command already looks in the transcript. `sendSystemNotification` deliberately never runs this check — only user-typed composer input can be a slash command the console intercepts.

**Deliberately excluded, not merely unimplemented.** Two SDK-recognized slash commands are excluded from the `claude-sdk` row on purpose, so a future reader does not re-add them just because the SDK's own `system:init.slash_commands` advertises them — see `EMBEDDED_AGENT_SLASH_COMMANDS`'s doc comment for the full reasoning:

- **`/clear`** — the SDK honours it, but resets the SDK's OWN conversation state with nothing on our side to match; offering it would invite the console's persisted transcript to silently diverge from what the SDK actually remembers. `sdk-engine.ts` maps an observed `conversation_reset` (the SDK's own signal for this) to a `turn-error` that declares the divergence instead — see [embedded-agent-sdk-engine.md §4](embedded-agent-sdk-engine.md#4-the-message-mapping) for the mechanism.
- **`/model <name>`** — the SDK honours it only for the current session (does not persist), which conflicts with [agent-surface.md](agent-surface.md) Ruling 3's worker-persisted-override contract. Model persistence is tracked separately (#1521 Phase 3).

**Client contract (frontend half, tracked as a separate follow-up task; documented here for that task's implementer).** `MessagePanel`'s `slashCompletionEnabled` composer affordance is driven by `EMBEDDED_AGENT_SLASH_COMMANDS[worker's engine]`, offering only the entries present there — never a hardcoded or SDK-advertised list. Two client-side behaviors this table's shape implies but does not itself enforce:

1. **Only offer what's in the table.** The completion source lists exactly the entries for the worker's engine, regardless of what the engine might additionally recognize (e.g. never offering `/clear`/`/model` per the exclusions above).
2. **Unknown-slash notice.** When a user sends a message beginning with `/` that does NOT match any entry in the table for that worker's engine, the client shows a non-blocking notice that the text will be sent as ordinary prose rather than interpreted as a command — this is the safety net for `claude-sdk`, which forwards ANY text unconditionally (there is no console-side gate on that engine) and could otherwise silently swallow a mistyped or unsupported command (e.g. `/model`) into the model's next turn with no indication anything unusual happened.
...
**Denominator.** `EmbeddedAgentDefinition.contextWindowTokens?: number` (migration v27) is the server-side source the ratio's denominator is resolved from. As of #1556, the value the CLIENT renders travels not through the `embedded-agent-created` / `embedded-agent-updated` registry broadcasts but on the worker's own wire field (`EmbeddedAgentWorker.contextWindowTokens`, server-composed by `resolveEffectiveContextWindow`) — see [UI](#ui)'s "Denominator source" for the client-side wiring and the rationale. When unset, the client shows raw token counts with no ratio and no color escalation, and — per the threshold semantics above — auto compaction cannot fire.

### Compaction prompt loader

Module `packages/embedded-agent/src/compaction-prompt.ts` (renamed from `handoff-prompt.ts`), deliberately a narrower cousin of the [Instruction loader](#instruction-loader), not a call into it — the semantics differ (override, not concatenation):
...
**Denominator source (#1556).** The bar's `contextWindowTokens` denominator is `EmbeddedAgentWorker.contextWindowTokens`, a field on the worker's own wire shape composed server-side by `resolveEffectiveContextWindow` (agent-surface.md Ruling 4) — never looked up client-side from the embedded-agent definition registry. `EmbeddedAgentWorkerView` and `ContextUsageBar` both take it as a prop threaded down from `SessionPage`'s `resolveActiveEmbeddedContextWindowTokens`. This keeps the client structurally unable to fall back to the definition's value for a worker whose effective window has since diverged from it (the model-override phase, #1554).
...
**This is measured, not merely a possibility, and it is structurally un-fixable from this repo's side.** `claude-sdk`'s compaction is SDK-native — `sdk-engine.ts` drives it by sending a `/compact` slash command into the SDK session, which runs Anthropic's own internal summarization — and this repo supplies no compaction/summarization prompt to it at all. There is therefore no hook a preservation instruction (or anything else) could attach to, even in response to the finding above. The `#1400` drop is a confirmed, real, non-deterministic loss under **explicit verbatim-preservation coaching** in the prompt, on a path this repo cannot influence.

**A same-investigation baseline for `openai-api` ([#1350](https://github.com/ms2sato/agent-console/issues/1350) comments, 2026-08-30).** Ten independent runs, three planted conversation-local identifiers each, stated as ordinary facts with **no preservation coaching** — the opposite of #1400's explicit "must be preserved verbatim" wording — retained all 30/30 identifiers verbatim, at ~44K real (non-estimated) prompt tokens per run. That is a much smaller pressure regime than #1400's ~95k-token runs, so the two results are **not a cross-engine comparison** — different regimes, not comparable numbers. See the Issue's comments for the full measurement record and disposition (re-scoped, not closed: a larger-scale `openai-api` run remains unmeasured).
...
**Bulk restart reaches an embedded-agent worker only when it has a live process.** "Restart all agents" restarts an ACTIVE embedded-agent worker by calling the same `deactivate()` then `activate()` this section already describes — not a new mechanism, and not a special case: `activate()` always attempts Restore at [step 4](#server-side-management-embeddedagentworkerservice) above, so this reactivation gets whatever Restore already does for any other activation, automatically. A DORMANT (evicted) embedded-agent worker, or one whose eviction has committed but not yet finished exiting (`isEvicting`), is left alone and reported as skipped: reactivating either would be indistinguishable from waking a worker the eviction decision above just chose to stop, undoing it.
...
4. **Attempt restore before resetting**, unless this is the worker's first-ever activation (empty persisted output file -- nothing to restore, proceed with today's v1 reset unconditionally):
   ...
   - **4f.** On failure at any step (unparseable stream, a reconstruction invariant violated, an I/O error): **preserve-and-declare in place is now PRIMARY, and the v1 reset-and-sidecar behavior is the FALLBACK** ([#1447](https://github.com/ms2sato/agent-console/issues/1447) stage 4, inverting the ordering #1449 established) -- see [Failure invariant](#failure-invariant-restore).
   - **4f-i. A rotation fragment at the head is skipped, under two conditions, and only the first record ever is.** Restore reads the live output window from its base, and rotation cuts that file at a byte offset, so the window can open on the tail of a record whose head was archived. The cut is newline-aligned as of [#1445](https://github.com/ms2sato/agent-console/issues/1445), but every worker rotated before that still carries such a window on disk, and the alignment has a documented fallback that produces one deliberately.

     **Condition 1 — the caller must say where the assembled stream begins.** `reconstructConversation` takes a `RestoreStreamAnchor` — `'true-start' | 'boundary' | 'pruned' | 'cap'` — supplied by the walk-back (below). The reader sees only text and cannot tell a rotation fragment from corruption; the caller always knows. **A malformed first record in a stream anchored at `'true-start'` is corruption and still fails 4f** — that anchor means offset 0, where a partial first record cannot be a rotation artefact.

    **The anchor is passed whole rather than reduced to a boolean, and that is a correctness point rather than a style one.** Two independent facts are derived from it: *may the first line be a partial record* (`anchor !== 'true-start'`) and *does the stream begin somewhere the system can stand behind* (`'true-start'` or `'boundary'`). A boolean would admit a fourth combination — reaches the true start *and* may open on a fragment — that cannot occur, and the walk-back is exactly what separated the two facts: a stream stopping at a compaction boundary reaches an honest head **and** can still open mid-record, because a segment's start is a previous cut point and the cut's documented no-usable-newline fallback leaves such a point partial.

     **Condition 2 — the stream must begin at an anchor the system can stand behind**, which the walk-back is what provides.

    **4f-ii. Do not leave the window cut ([#1202](https://github.com/ms2sato/agent-console/issues/1202)).** Reading only the live window was the defect, not the starting point for a judgment about cut windows. A worker whose compaction boundary had rotated into an archived segment reconstructed from a conversation beginning in the middle — and because the cut lands on a record boundary, **nothing failed**: it came back silently shorter, with no error, no marker, and no signal at any layer. A reset is loud; that was not.

    So restore assembles its input by walking **back** through archived segments, newest-first, with four stopping conditions:

    | anchor | meaning | restore |
    |---|---|---|
    | `boundary` | a compaction boundary is in range | full, from an already-declared discard |
    | `true-start` | the stream begins at offset 0 | full, from the real beginning |
    | `pruned` | retention already deleted the oldest segment | **partial**, from the first `user-message` |
    | `cap` | the walk hit its byte ceiling (`WORKER_OUTPUT_RESTORE_MAX_BYTES`) | **partial**, identically to `pruned` |

    **The fast path reads no archive at all**: a live window already containing a boundary returns immediately, byte-identical to what restore read before the walk-back existed.

    **A second consumer of the same storage had the identical blind spot, one layer up ([#1506](https://github.com/ms2sato/agent-console/issues/1506)).** The walk-back above fixes restore's INPUT -- what the loop reconstructs its conversation from. It says nothing about the client's INITIAL DISPLAY -- what a reconnecting browser tab is shown. Those are two different readers over the same segmented storage, and before #1506 only one of them walked the archive: `readLastNLines` (the initial-load read `getWorkerOutputHistory` calls for a client's `request-history` at `fromOffset` 0/undefined) returned the live window alone, exactly like restore did before #1202. A heavily-rotated worker therefore reconstructed its conversation correctly (restore's fix) while a reconnecting client's transcript rendered `"No messages yet"` (the display reader's own, unfixed, live-window-only shape) -- the model recalling a fact whose displayed evidence had vanished.

    **The fix is a shared primitive, not a shared method.** `readHistoryForDisplay` (server's `worker-output-file.ts`) walks archived segments newest-first using the exact same decompress/ENOENT/cap machinery `readHistoryForRestore` uses -- both call a private `walkArchiveSegmentsBackward` that takes a caller-supplied `shouldStop` predicate and knows nothing about *why* either caller is walking. **What the primitive shares stops at the walk mechanics; the two callers are independent along BOTH of the axes that decide what happens around it** -- not only the stop condition (below), but also the error policy for a segment that fails to decompress (a truncated or corrupted gzip, not an absent one -- ENOENT is the separately-handled pruned edge). A future third caller of `walkArchiveSegmentsBackward` should expect to make its own decision on each axis rather than assuming either one generalizes from these two:

    | | restore (`readHistoryForRestore`) | display (`readHistoryForDisplay`) |
    |---|---|---|
    | stops when | a compaction boundary, the true start, a pruned/absent segment, or the byte cap | enough LINES are assembled to satisfy `WORKER_OUTPUT_INITIAL_HISTORY_LINES` (or segments/the byte cap are exhausted) |
    | the verdict the caller needs | which of four SEMANTIC anchors it reached (`reconstructConversation` branches on this -- see Condition 1 above) | none -- a display consumer renders whatever was assembled, trimmed to the line budget, with no distinction between "filled the budget" and "ran out of archive trying" |
    | the byte ceiling | `WORKER_OUTPUT_RESTORE_MAX_BYTES` | `WORKER_OUTPUT_DISPLAY_FILL_MAX_BYTES` -- a DISTINCT config knob, not a reused one, because "how much reconstruction context is safe to seed a conversation with" and "how much archive I/O one client's initial-load request may cost" are different questions that happen to share a default today |
    | on a damaged (non-ENOENT) segment | propagates -- `runActivation` has a verdict to act on it (the destructive reset + sidecar preservation); handing the model a silently-degraded partial conversation is the dangerous outcome, so restore must know the walk did not complete honestly | degrades to the live window it already had in hand before the walk started (a `try`/`catch` local to `buildDisplayWindow`, CodeRabbit finding on PR #1510) -- a partial transcript is display's normal, expected shape (it is already showing a line-budget-truncated view by design), so going silent about the *transcript*'s completeness is not a new failure mode; the corruption *event* itself is still logged (with the segment's identity) at the point `walkArchiveSegmentsBackward` catches it, so it stays discoverable server-side even though the client sees nothing different from an ordinary truncated view |

    A single method taking a `mode: 'restore' | 'display'` flag was considered and rejected -- these are two independent fact-sets under one name, the same shape already rejected twice elsewhere the same week (#1202's amendment 4; the #1434 field split). Restore's anchor is consumed by `reconstructConversation` as a *correctness* input (Condition 1 above: the reader cannot tell a rotation fragment from corruption without it). Display's stop reason is consumed by nothing -- a browser tab does not need to know whether its transcript is complete because the live window sufficed or because the archive ran out; it just renders what it received. Collapsing two callers with different consumption needs behind one flag would have coupled restore's correctness-input contract to display's fire-and-forget one, for no caller's benefit. The error-policy row above is the same shape one layer down: a shared `mode` flag would have coupled restore's "propagate, because a consumer downstream decides" to display's "degrade, because there is no such consumer" -- two callers, two policies, one primitive that stays agnostic to both.

    **The shared primitive's one genuinely shared property is where the lock is taken.** Both callers assemble the live window, the manifest, and the archived segments inside ONE `runExclusive` critical section -- the same #1493 snapshot-consistency property this section's 4f-ii already established for restore, for the same reason: a flush cutting the live file between two separate lock acquisitions would leave the live data overlapping a segment the newer manifest lists, and a display response would then either duplicate or skip a line. `readHistoryForDisplay` does not read the live window under one acquisition and the archive under a second one -- see [#1497](https://github.com/ms2sato/agent-console/issues/1497) for the analogous gap this deliberately does not reopen (`runActivation`'s two-call shape, out of scope for #1506).

    **Why the client had no fallback of its own to lean on.** Terminal/PTY workers solve exactly this initial-window problem client-side: `terminal-store.ts`'s scroll-to-top-triggered `requestOlderHistory()` pages older ranges on demand via `readHistoryRange`, so even a live-window-only initial response is merely a starting point, not the whole story. The embedded-agent chat client has no such affordance in v1 (`embedded-agent-store.ts`'s `history-range` case is a deliberate no-op -- see the file's own comment), so its initial response IS the whole story for that client. That asymmetry is why `getWorkerOutputHistory`'s routing is scoped to `worker.type === 'embedded-agent'` rather than applied to every stream worker: PTY workers do not need an archive-aware initial read because they already have a paging path that reaches the archive; embedded-agent workers, until a paging affordance exists for them too, do not.

    **Why partial restore from the first `user-message` is honest rather than merely tolerant.** The transcript replay begins from the same available head, so **memory and display begin at the same point and agree** — there is no divergence for C2 to declare. And both remaining anchors are *declared* discards: retention is one the operator configured, the cap one we configured. Neither is a rotation's unintended byte cut. Debris before that first `user-message` is dropped rather than presented, so a mid-turn `tool-result` cannot lead.

    **A stream with no `user-message` in reach still fails 4f** — there is no honest place to begin, so the reset with its sidecar remains the answer.
...
Concretely, a `tool-call` arriving with no assistant message open **opens an implicit empty one** and adopts the writer's own flush into it when that arrives, rather than starting a second message -- so one assistant turn never splits in two, and the two orders converge on one reconstruction. That tolerance is gated on **a turn having begun in the window** (a `user-message` already replayed). The two cases are indistinguishable at the `tool-call` itself, which is why the discriminator is the turn's beginning rather than anything about the call.

**The walk-back changed how often that guard is reached, not what it means.** It was written when restore read the live window alone, so debris from a mid-turn cut arrived routinely; now the assembled stream starts at a boundary, the true start, or the first `user-message` of a partial restore, and in all three a turn has begun by construction. The guard remains as the belt: it still catches debris arriving by any route the walk-back does not cover, and a `tool-call` leading a window is still not something to synthesise a turn around.
...
| Noise (replay-only, contributes nothing) | `assistant-delta`, `assistant-thinking-delta`, `state`, `context-usage`, `ready`, `exited`, `turn-error`, `fatal`, `sdk-session-id`, `sdk-resume-failed` (R1), `turn-interrupted` (R1), `restore-failure-declaration` (R6, #1447 stage 4) | Skipped. Reasoning/thinking content is never part of the conversation array even live ([The loop's turn cycle](#the-loops-turn-cycle)), so restore does not reconstruct it either. A `turn-error` (or `fatal`) immediately following an unresponded `tool-call` is the expected wire trace of a live-aborted turn -- the runtime repair that ran at the time was in-memory-only and never reached the wire (row 7 of the [parts cross-reference](#runtime-abort-repair-vs-restore-time-repair-parts-cross-reference) table). Restore-time repair (4d) heals this the same way the live abort would have; 4c does not need to special-case `turn-error`/`fatal` presence. `sdk-session-id`/`sdk-resume-failed`/`turn-interrupted` are bookkeeping about the PROCESS's history, not about what was said. `restore-failure-declaration` is deliberately Noise here -- restore-TRANSPARENT by design, the opposite of the boundary row below |
| Boundary (handled by 4b, not 4c) | `context-compacted`, legacy `context-handoff`, and `restore-failure-boundary` (R2, #1447 stage 4) | Never replayed into the array by 4c -- consumed by 4b to locate the restore window's start, per [Compaction boundary](#compaction-boundary) and [Failure invariant](#failure-invariant-restore)'s primary path |
...
**Correction (architect review, PR #1191 R1).** An earlier draft of this subsection claimed the v1 reset fallback already preserves pre-reset bytes via the Archive Segment machinery. That premise is false against the actual code: `WorkerOutputFileManager.resetWorkerOutput` (`packages/server/src/lib/worker-output-file.ts`) calls `deleteContentFiles`, which deletes the live file, the legacy compressed file, AND every archived segment, then writes a fresh empty manifest plus an empty live file. The Archive Segment mechanism ("Replaces destructive truncation", glossary) is a separate code path that fires on live-file size-overflow rotation -- it does not run before an activation reset. `resetWorkerOutput` is unconditionally destructive; nothing precedes it that archives the bytes it is about to delete.

**Inverted ([#1447](https://github.com/ms2sato/agent-console/issues/1447) stage 4, R1): preserve-and-declare in place is PRIMARY; the v1 reset-and-sidecar behavior below is the FALLBACK, reached only when the primary path's own write fails.** #1449 first closed the "restore failure discards the record" gap with the sidecar described below, kept as v1's degrade-to-proven-safe behavior; #1445's stages 1-3 then reduced how OFTEN a genuinely unrecoverable log occurs; this stage inverts which of the two paths runs first, because C1 ("display completeness") forbids a **deliberate discard** that was never declared to the user, and a reset's sidecar is exactly that -- manifest-invisible, never client-offered. Retention is a declared discard the operator configured *ex ante*; a reset's declaration is what this stage adds *ex post*, via the marker below and (on the fallback route) the R6 persistent row.

**Primary path: preserve-and-declare.**

- On any restore-step failure (unparseable stream, a reconstruction invariant violated, an I/O error), the server attempts to append a `restore-failure-boundary` marker event to the SAME live output stream, durably, via `WorkerOutputFileManager.appendRestoreFailureMarker` -- a direct, non-buffered write that PROPAGATES an I/O failure to the caller (unlike `bufferOutput`'s deferred, error-swallowing flush). No reset. No new epoch. Every byte the worker had persisted, including the corrupt/unparseable region that caused the failure, is retained on disk and remains readable through the ordinary history read (C1).
- `restore-failure-boundary` is a NEW persisted event type (`EmbeddedAgentServerEvent`, `packages/shared/src/types/embedded-agent.ts`), a reconstruction BOUNDARY of the same class as `context-compacted` (`packages/embedded-agent/src/restore.ts`'s `BOUNDARY_EVENT_TYPES`): the next restore attempt must never read past it into the region that preceded it. Unlike `context-compacted`, it carries NO `summary` field -- there is no summary to carry forward, since memory starts from nothing at this boundary. `reconstructConversation` seeds a window opening here with `buildRestoreFailureSeedMessages` (system prompt only, `packages/embedded-agent/src/conversation-seed.ts`), never `buildCompactionSeedMessages` -- the marker's own type has no `summary` field, so calling the compaction seed builder on it is a compile-time error, not a review convention.
- **The region before the marker may itself be unparseable, and that is precisely why the marker exists.** A window opening at a boundary is ordinarily read top-to-bottom by `parseStreamEvents`, which fails loudly on any malformed line after the first (by design, elsewhere in this section). A restore-failure-boundary marker is different: `reconstructConversation` first calls `trimToLastRestoreFailureBoundary`, which locates the LAST such marker in the raw text via a best-effort scan and slices the input to begin exactly at that line -- BEFORE `parseStreamEvents` ever runs. The corrupt region is therefore never handed to the strict parser at all ("never re-parsed for memory"); it is still fully present in the underlying file and fully replayed for DISPLAY (the client's per-line-tolerant renderer), just excluded from what feeds the model's next turn. This is what turns a single corrupt region into a ONE-TIME loss rather than a permanent restore-failure loop: the SECOND restart after a failed restore reconstructs from the marker onward and succeeds.
- No epoch bump. `worker.epoch`/`worker.outputOffset` sync to the marker append's own manifest read (mirroring the success path's `currentEpoch`/`currentOffset` sync) -- the epoch is the file-reset generation counter, and a file that was never reset keeps its generation.

**Fallback path: reset-and-sidecar (v1 behavior, unchanged), reached only when the marker append above itself fails** (e.g. an I/O error on the same volume that caused the restore failure in the first place):
...
- **Restore-failure sidecar.** Immediately before invoking `resetWorkerOutput`, best-effort-rename the CURRENT live output file (not the manifest, not any already-archived segment) to a **fixed** name, `<workerId>.restore-failed.log`, in the same worker directory, overwriting any prior file of that name -- this is genuinely single-slot (a name keyed on the old epoch would accumulate one file per failed generation instead of retaining only the most recent; the epoch of the preserved bytes is recoverable from the log content itself, so the filename does not need to carry it). The rename runs INSIDE `WorkerOutputFileManager`'s per-key exclusive domain (an option on `resetWorkerOutput`) -- never as a bare caller-side `fs.rename`. `resetWorkerOutput` already drops any pending flush for the same key inside that lock; a rename outside the lock would race a pending flush recreating the live file between the rename and the reset, silently losing exactly the bytes the sidecar exists to keep. (Corollary: any buffer content not yet flushed to the live file at the moment of rename is, by the nature of a best-effort preservation step, not in the sidecar either -- an accepted limitation, not a defect.)
  - **Not manifest-referenced, never client-visible.** The sidecar is not listed in the manifest and is never replayed or offered over the worker WebSocket protocol -- it is inert, diagnostic-only storage. This is precisely the invisibility C1 forbids as the PRIMARY outcome, which is why stage 4 demoted it to a fallback rather than removing it: it remains the correct degrade-to-proven-safe behavior for the rare case where even the in-band write fails.
  - **Not automatically consumed by a later restore attempt.** The next activation runs the normal restore-then-reset flow against the (now empty, post-reset) live file exactly like any other worker; recovering the sidecar's content is a manual/operational action (an operator reading the file directly), not an automated retry path.
  - **Best-effort, never blocking.** If the rename itself fails, log it and proceed with the reset regardless. The invariant is "preserve when possible, never let preservation block or replace the reset" -- blocking the reset on a failed preservation attempt would trade a data-loss hazard for a worse one, a permanently wedged worker.
  - **Scope limitation.** The sidecar preserves the live file only. If the worker's stream had already rotated into archived segments before the failed restore attempt (an overflow rotation, unrelated to restore), those segments are still deleted by `deleteContentFiles` exactly as today.
- The fallback path is otherwise byte-identical to v1's unconditional activation step 4: `resetWorkerOutput` (fresh epoch + truncate), run immediately after the sidecar rename attempt (whether or not that attempt succeeded).
- **R6 persistent divergence declaration ([#1447](https://github.com/ms2sato/agent-console/issues/1447) stage 4, Issue [#1492](https://github.com/ms2sato/agent-console/issues/1492)).** A destructive reset never touches `worker.sdkSessionId`, so a `claude-sdk` worker's own SDK-side session store can still remember the discarded conversation after the fallback path runs -- and the incarnation-scoped `restore-info` failure form's declaration of that fact dies with the incarnation, while the SDK's memory persists. When the fallback path runs for a `claude-sdk` worker whose `sdkSessionId` survives the reset, `resetWorkerOutput` writes a `restore-failure-declaration` event (`EmbeddedAgentServerEvent`) as the SOLE content of the fresh (post-reset) live file, in place of leaving it empty (`opts.persistentMarkerLine`). `openai-api` fallback resets write no such row -- reconstruction IS that engine's memory, so a Loss there is already symmetric and already declared by the failure form. This row is restore-TRANSPARENT (the OPPOSITE of the primary path's boundary marker, and deliberately so): it declares an asymmetry reconstruction must IGNORE (ordinary Noise in `replayWindow`, never a boundary), not a discard reconstruction must respect. It survives every subsequent incarnation until the next reset, which is the point: the declaration must outlive the incarnation that discovered it, because the divergence does.
- **The restore failure is declared to the client (#1449), on BOTH routes.** The server sends the `restore-info` failure form over the same channel the success form uses -- see [UI](#ui) below for the full contract, the D1/D2/Loss derivation rule, and stage 4's `preservation` field (R4), which tells the client which of the two routes above actually ran.

### UI

**Delivery mechanism: `restore-info` (worker WS envelope, not the stdio/persisted-NDJSON protocol).** The UI behaviors below are driven by a `WorkerServerMessage` variant that is now a **discriminated union of two forms** (`packages/shared/src/types/session.ts`) -- NOT a new `EmbeddedAgentEvent`/`EmbeddedAgentServerEvent`/persisted-NDJSON row (the "no new persisted/wire EVENT type" statement above is scoped to the stdio protocol / `EmbeddedAgentStreamEvent`; the worker WS outer envelope is a separate layer that already carries transient, non-persisted pushes like `activity`/`error`):

- **Success form**, unchanged from #1123/#1205: `{ type: 'restore-info'; epoch: number; restoredMessageCount: number; repairedToolCallIds: string[]; completed: boolean; sdkResumed?: boolean; failed?: false }`.
- **Failure form (#1449, extended #1447 stage 4)**: `{ type: 'restore-info'; epoch: number; failed: true; sdkResumed?: boolean; preservation?: 'in-band' | 'sidecar' | 'lost' }`. `preservation` (R4) discriminates which of [Failure invariant](#failure-invariant-restore)'s two routes ran: `'in-band'` (the primary path -- the transcript is still the visible display, no reset), `'sidecar'` (the fallback path, and the rename succeeded), `'lost'` (the fallback path AND the rename itself failed -- nothing was preserved anywhere). **Absent means a pre-stage-4 server** -- optional rather than defaulted, a wire-compat requirement so an old server's client renders today's unconditional copy unchanged.

`restore-info` is now sent when restore SUCCEEDS (4e) **or FAILS (4f)** -- never on a first-ever activation (4-first-time), where there is nothing to declare. This supersedes the earlier "sent ONLY when restore succeeded... restore failure sends nothing extra, exactly v1's silent-degradation behavior" statement this paragraph made before #1449: a restore failure is no longer silent.

### Failure form: what it declares, and the D1/D2/Loss derivation rule (#1449)

Per [#1447](https://github.com/ms2sato/agent-console/issues/1447)'s "C2 refinement," a divergence between what the model remembers and what the display shows has a **direction**, and the wire message must not conflate them:

- **D1 -- display ahead of memory.** A transcript exists that the model does not know about. This is what the SUCCESS form's `sdkResumed: false` already means (Phase 3 / R1's existing divergence notice, unchanged by this Issue).
- **D2 -- memory ahead of display.** The model still remembers the conversation; the transcript is gone. Reachable only on `claude-sdk`: `worker.sdkSessionId` survives the restore catch untouched, and the failed incarnation's `init` command still asks for a `resume` keyed on that id (gated on `resumeId !== null`, independent of whether the SAME activation's restore attempt succeeded or failed) -- so the SDK resume may still carry the conversation forward even though the transcript could not be reconstructed.
- **Loss -- both are gone.** Reachable only on `openai-api`: reconstruction from the persisted log IS that engine's memory, so a restore failure there is a genuine, symmetric loss. There is no separate "resume" concept to fall back on.

**The wire message states WHAT HAPPENED (`failed: true`, optionally `sdkResumed`); it does not itself state D2 vs Loss.** The client is responsible for deriving the direction from the message's `engine` (known from the worker/definition, not carried on `restore-info` itself) and `sdkResumed`:

| Engine | `sdkResumed` on the failure form | Client derivation |
|---|---|---|
| `claude-sdk` | `true` (optimistic; resume was attempted, not yet known to have failed) | D2 |
...
**Copy is now CONDITIONED on `preservation` (R4, #1447 stage 4) rather than unconditional -- the banner never claims what did not happen.** The pre-stage-4 wording asserted "a diagnostic copy of the record has been preserved" regardless of whether the sidecar rename that backed that claim actually succeeded; stage 4 makes preservation in-band-displayable, and therefore verifiable, so the copy must agree with what `preservation` reports:

- **D2, `preservation === 'in-band'`**: "This worker's earlier conversation is still shown above. The agent does not carry it forward from here, but it may still remember it independently." (The transcript IS the display; there is no separate "copy" to reference.)
- **D2, `preservation === 'sidecar'` or `'lost'`**: "This worker's earlier conversation could not be restored for display, but the agent may still remember it." -- appending ", and a diagnostic copy of the record has been preserved" only when `preservation === 'sidecar'`.
- **Loss, `preservation === 'in-band'`**: "This worker's earlier conversation is still shown above. This turn starts fresh."
- **Loss, `preservation === 'sidecar'` or `'lost'`**: "This worker's earlier conversation could not be restored — a diagnostic copy of the record has been preserved. This turn starts fresh." -- with the diagnostic-copy clause dropped when `preservation === 'lost'`.
- **`preservation` absent (pre-stage-4 server)**: today's unconditional copy, verbatim, both directions -- the wire-compat fallback.

Exact strings are a client-implementation decision, reviewed at PR time; the rule above is the contract every variant must satisfy.

The Loss sentence is a single shared constant rendered by both routes that reach it (`openai-api`'s own restore failure, and a `claude-sdk` restore failure whose SDK resume also failed) -- only the D2-vs-Loss branch DECISION is engine-dependent, never the wording. `restoreDivergedD2`/`restoreLoss` are gated on the CONFIRMED `isSdkEngine`/`isOpenaiApiEngine` checks the view already uses for the two existing notices, not on `!isSdkEngine`, for the same three-valued-engine reason those notices avoid it: `!isSdkEngine` is also true during the still-unresolved window (registry loading, or a dangling/unmatched `embeddedAgentId`), which would show a direction before the engine is even known.

**[#1447](https://github.com/ms2sato/agent-console/issues/1447) stage 4 extends this failure form rather than replacing it**, exactly as designed: the declaration channel itself did not change, only its `preservation` field (R4, above) and what gets preserved before the declaration fires (the [Failure invariant](#failure-invariant-restore) primary/fallback inversion). The `restore-info` failure form and the D1/D2/Loss derivation rule are unchanged by this stage.
...
1. **Fast-path push, either form.** Immediately after the restore attempt (4a-4d, success OR 4f, failure) completes -- before the subprocess is spawned -- the server broadcasts `restore-info` to every currently-attached `connectionCallbacks` entry for the worker (`EmbeddedAgentWorkerService`, mirroring `broadcastActivity`'s fan-out): the SUCCESS form with `completed: false` (the new incarnation has not reported `ready` yet), or the FAILURE form (#1449) when the restore attempt threw. Reaches zero listeners when nobody is watching yet; not a correctness requirement.
2. **Bootstrap re-delivery (authoritative), either form, current value.** The server retains the current incarnation's restore result -- whichever form it is -- in the worker's runtime state for the lifetime of that incarnation (subprocess alive). EVERY new WS connection during that incarnation -- not only the one that triggered the activation -- receives `restore-info` again as part of its bootstrap, alongside its initial `history` response, carrying the CURRENT retained value (for the success form, whichever `completed` value is currently true). This is what makes the note durable across reconnects: a client that connects (or reconnects) after the fast-path push already fired still learns about the restore -- success or failure -- and a client connecting AFTER a successful incarnation is already ready correctly sees `completed: true` immediately (no stuck loading state).
3. **Ready-triggered completion repush, `completed: true` (Issue #1205) -- SUCCESS FORM ONLY.** The server observes the new incarnation's `ready` NDJSON event server-side (`EmbeddedAgentWorkerService`'s stdout line handler, the same place [Initial prompt delivery](#initial-prompt-delivery) hooks). The first time `ready` is observed for an incarnation whose retained `restoreInfo` is the SUCCESS form with `completed: false`, the server flips that incarnation's retained `completed` flag to `true` and re-broadcasts `restore-info` (same shape, `completed: true`) to every currently-attached connection, using the identical fan-out as step 1. This is the ONLY signal that ever flips the client's loading state off -- it is authoritative because it is derived from the server having actually observed `ready`, not from client-side replay-order guessing. **A failed-restore incarnation's later `ready` never triggers this step** -- the failure form has no `completed` field to flip, and the retained value is left completely untouched.

Both forms also participate in the **resume-correction push (R1)**: when a `claude-sdk` incarnation's SDK resume is later found not to have taken (`sdk-resume-failed` or a session-id mismatch), the server corrects the retained `restoreInfo`'s `sdkResumed` to `false` and re-broadcasts, using the identical fan-out. This is the SAME mechanism the success form's divergence notice already relies on (R1/#1410) -- unchanged for #1449 -- and it is what resolves a failure form's optimistic `sdkResumed: true` (D2) down to `false` (Loss) once the resume outcome is actually known.
...
- **The transcript row.** The `exited` row renders as *"Agent process exited (code: 0)"*, a plain historical fact with no interactive affordance — the next message wakes the worker by itself, so there is nothing the user needs to instruct. (**R1 (#1455)**: this was originally a **Restart** button on every such row, which meant every past exit kept offering the action even after a later restart had already superseded it. The Restart action now lives outside the transcript entirely, as a sibling element driven by the worker's CURRENT exit state — see [UI](#ui) — rather than by which row happens to be last.) An evicted row renders instead as a quiet line with no affordance, same as any other `exited` row. (`stderrTail` is never attached on an evicted exit either, per #1454 -- the same no-affordance-for-a-routine-exit reasoning applies to withholding a disclosure that would have nothing diagnostic to say.)
