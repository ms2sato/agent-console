...
4. **Attempt restore before resetting**, unless this is the worker's first-ever activation (empty persisted output file -- nothing to restore, proceed with today's v1 reset unconditionally):
   ...
   - **4f-i. A rotation fragment at the head is skipped, under two conditions, and only the first record ever is.** Restore reads the live output window from its base, and rotation cuts that file at a byte offset, so the window can open on the tail of a record whose head was archived. The cut is newline-aligned as of [#1445](https://github.com/ms2sato/agent-console/issues/1445), but every worker rotated before that still carries such a window on disk, and the alignment has a documented fallback that produces one deliberately.
     ...
     **Condition 1 — the caller must say the file rotated.** `reconstructConversation` takes a `truncated` flag, supplied from `readHistoryWithOffset`'s `startOffset` (`liveBaseOffset > 0`). The reader sees only text and cannot tell a rotation fragment from corruption; the caller always knows. **A malformed first record in a file that was never cut is corruption and still fails 4f** — this layer is the only thing that detects it, and an unconditional skip would trade that away.

     **Condition 2 — the surviving window must contain a compaction boundary.** If it does not, 4f applies unchanged. Skipping the fragment is not by itself enough to make a restore honest: the window still opens wherever the byte cut fell, so restoring would present a partial conversation as a whole one. **A compaction boundary is an already-declared discard**, so slicing there drops the mid-turn debris and starts the window at a turn boundary — honest in [#1447](https://github.com/ms2sato/agent-console/issues/1447) C2's sense. Whether a boundary-less rotated window should partially restore instead of resetting is [#1202](https://github.com/ms2sato/agent-console/issues/1202) Gap 1's judgment, and this rule deliberately leaves it open rather than settling it as a side effect.
