...
4. **Attempt restore before resetting**, unless this is the worker's first-ever activation (empty persisted output file -- nothing to restore, proceed with today's v1 reset unconditionally):
   ...
   - **4f-i. The first record of the window may be a rotation fragment, and only the first.** Restore reads the live output window from its base, and rotation cuts that file at a byte offset -- so the window can open on the tail of a record whose head was archived. That first record, and no other, is dropped rather than failing the reconstruction. The cut is newline-aligned as of [#1445](https://github.com/ms2sato/agent-console/issues/1445) so new rotations do not produce one, but every worker rotated before that still carries one on disk, and the alignment has a documented fallback that produces one deliberately.

     **The tail stays strict, and the asymmetry is the contract rather than an implementation detail.** A malformed record anywhere after the first is not explained by the cut -- the only writer appends whole lines -- so it signals real damage, most likely a process killed mid-write, and must keep routing to 4f. Making the tail tolerant would convert every truncation into a silent partial restore. The allowance is positional, so a well-formed first record spends it: this is "the first record may be partial", not "one bad line anywhere".

     Both failure modes are covered, not only the parse throw: a cut can leave bytes that happen to be valid JSON without being an event, and a tolerance keyed on `JSON.parse` alone would leave that case failing for a reason a reader would have to reconstruct. Reaching the mid-turn window this exposes is [#1447](https://github.com/ms2sato/agent-console/issues/1447) C1's subject; what happens once it is reached (orphan -> reset, versus partial restore) is [#1202](https://github.com/ms2sato/agent-console/issues/1202) Gap 1's, and is unchanged here.
