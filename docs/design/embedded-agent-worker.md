...
**Bulk restart reaches an embedded-agent worker only when it has a live process.** "Restart all agents" restarts an ACTIVE embedded-agent worker through this same ordinary deactivate-then-reactivate path — so Transcript Restore applies to it exactly as it would to any other revival — but a DORMANT (evicted) embedded-agent worker is left alone and reported as skipped, because reactivating it on the caller's behalf would be indistinguishable from waking it, which defeats the eviction decision above. There is no third, restart-specific mechanism: the same two entry points this section already describes are the only way in.
