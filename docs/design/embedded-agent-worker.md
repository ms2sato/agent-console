...
**What `restoredMessageCount` counts, and on both engines (Issue [#1428](https://github.com/ms2sato/agent-console/issues/1428)).** It is **the number of restored entries originating from the persisted transcript — replayed messages plus a compaction summary — excluding only the synthetic system prompt.** The summary counts because it is restored content reconstructed from the `context-compacted` row, not a seed the reconstruction invents; the system prompt does not, because it is. So an empty transcript reports 0, and a worker killed immediately after a compaction reports 1. That second case is the edge that makes the definition worth stating rather than paraphrasing as "exclude the seed": under that paraphrase it would report 0 and **suppress the divergence notice exactly where the model has lost the most**.
