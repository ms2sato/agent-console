...
- **The restore failure IS now declared to the client (#1449), superseding the "silent degradation" statement this bullet previously made.** The server sends the `restore-info` failure form (`{ type: 'restore-info'; epoch; failed: true; sdkResumed? }`) over the same channel the success form uses -- see [UI](#ui) below for the full contract and the D1/D2/Loss derivation rule. The sidecar above is unaffected: it remains the diagnostic-only preservation step, and the wire declaration is a separate, additive change that announces the failure happened without changing what gets preserved. **This declaration is what [#1447](https://github.com/ms2sato/agent-console/issues/1447) stage 4 grows into a richer in-band preservation ("stage 4 changes what gets preserved, not how it's declared") -- the channel itself does not change again at that point.**
...
- **Failure form (#1449)**: `{ type: 'restore-info'; epoch: number; failed: true; sdkResumed?: boolean }`.
...
- **D2**: "This worker's earlier conversation could not be restored for display, but the agent may still remember it. A diagnostic copy of the record has been preserved."
- **Loss**: "This worker's earlier conversation could not be restored — a diagnostic copy of the record has been preserved. This turn starts fresh."
...
**This failure form is designed to survive into [#1447](https://github.com/ms2sato/agent-console/issues/1447) stage 4 as its declaration channel, unchanged.** Stage 4 changes WHAT GETS PRESERVED (the sidecar becomes in-band-displayable content instead of an invisible file), not HOW a failure is declared -- the `restore-info` failure form, its fields, and the D1/D2/Loss derivation rule above are the same contract stage 4's implementer extends rather than replaces.
