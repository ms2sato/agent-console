...
- Issue #878 phase 1: per-worker MCP bearer token, verification middleware, ownership checks — default `warn` in all modes until token delivery exists; Phase 4 flips the multi-user default to `enforce` (fail closed).
...
**Activation** (`activateEmbeddedAgentWorker(sessionId, worker, requestUser)`):
...
7. Observe `subprocess.exited` (after stream completion, mirroring `interactive-process-manager.ts:151-166`): append a server-authored `exited` event to the stream, set `subprocess = null`, revoke the MCP token, emit activity `idle`, and fire `onExit` callbacks (`'managed'` vs crash distinguished by whether a shutdown was requested).
...
// default: 'warn' in all modes; Phase 4 flips the multi-user default to 'enforce'
...
```
  ...
  **Fail-closed consequence for multi-user (Phase 4 onward):** at Phase 4 the multi-user default flips to `enforce`, so tokenless calls to the wired tools are rejected there — a worker (embedded or terminal) cannot opt out of identity verification by simply omitting its token. This is what makes the #878 guarantee real rather than advisory; it also means multi-user deployments require every agent path to carry a token, which is exactly what Phase 4 delivers before multi-user support is claimed. Until Phase 4 the default is `warn` in ALL modes: an `enforce` default is unsatisfiable while no token-delivery path exists (pre-Phase-4 it would halt every agent's MCP calls in any multi-user deployment the moment it deploys), and asking operators to manually override to `warn` in the interim invites forgotten fail-open state after the override stops being necessary. The flip is therefore guaranteed mechanically by Phase 4's acceptance criteria instead of left to deployment configuration. Single-user mode stays `warn` (the gap is operationally moot there, per Issue #878). The server emits a one-line startup notice when running multi-user in `warn` mode so the interim state is visible in logs.
...
- Worker creation: the session's "add worker" flow gains an embedded-agent option listing `EmbeddedAgentDefinition`s (from `GET /api/embedded-agents`), plus a link to create one when the registry is empty. Embedded-agent workers are NOT auto-created with sessions (unlike the git-diff worker, `session-manager.ts:620-623`).
