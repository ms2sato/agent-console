...
**One input shape, two independent implementations.** On `claude-sdk`, `TodoWrite` is the Claude Agent SDK's own NATIVE builtin tool — it is enabled simply by appearing in the `tools` allowlist passed to `query()` (`sdk-engine.ts`'s `buildOptions()`, [§4.1 of embedded-agent-sdk-engine.md](embedded-agent-sdk-engine.md#41-tool-surface--the-inverted-parity-question-ruled)); this repo implements no logic for it on that engine. On `openai-api`, it is a new builtin (`packages/embedded-agent/src/tools/todo-write.ts`), whose input is validated with **valibot** rather than the hand-rolled `parseArgs` pattern the other builtins use — deliberately pinned against a schema matching the SDK's own declared `TodoWriteInput` shape (`sdk-tools.d.ts`) field-for-field, so both engines' models see (and both engines' `tool-call` events carry) the exact same argument shape. That shape-parity is what lets the client render both engines through **one** component (`TodoPanel`, below) with no per-engine branch.

**Per-incarnation state, not a filesystem or server write.** The `openai-api` implementation holds "the last accepted list" as private closure state inside `createTodoWriteTool()` — a factory `packages/embedded-agent/src/tools/index.ts`'s `resolveEnabledBuiltinTools` calls fresh on every invocation, unlike the other six builtin tools (which are shared, stateless module-level singletons). This is what makes a second incarnation start with an empty list rather than inheriting the previous one's: each subprocess incarnation gets its own instance. No new persistence — the durable record of the task list is the transcript itself (the `tool-call` entries), not this in-memory state, which exists only to compose the `execute()` result string's item counts within a single incarnation.
