...
- **No third `turnActive` clear site.** Clearing stays in its two existing places — `state: 'idle'` and the exit observer. The fatal path *reaches* them; it does not become a third writer. A third writer breaks the single-writer property that made this defect findable at all.
