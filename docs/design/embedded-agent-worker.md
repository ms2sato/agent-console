...
**Correction (owner UX review, `fix/handoff-button-remove`).** Phase A's initial always-visible chrome button (`Start handoff`) was removed following owner UX critique (`fix/handoff-button-remove`). Manual handoff invocation is now banner-CTA-only (shown when the soft threshold is crossed). The `Handing off…` label described above was rendered only inside that removed chrome, so there is currently no visible in-flight indicator for a banner-triggered handoff -- `MessagePanel`'s existing Send-disabled/Cancel-shown behavior (via `cancelState`) is the only remaining in-flight signal. Reachability when `contextWindowTokens` is unset (dashed bar, no threshold banners), and the permanent placement of an in-flight indicator, are being reconsidered together by the architect and owner in follow-up Issue [#1206](https://github.com/ms2sato/agent-console/issues/1206).
...
- **Smoke (multi-user, before claiming multi-user support)**: `scripts/smoke/check-embedded-agent-elevation.ts`, importing the production spawn helper (never replicating argv), spawning as a real second user, asserting: loop starts (`bun` resolved via the configured `EMBEDDED_AGENT_BUN_PATH`, not PATH-only bare-name lookup inside the elevated login shell — Issue #1221), `init` handshake completes, the configured `EMBEDDED_AGENT_BUN_PATH` binary's `--version` matches the actual systemd server binary's version at the conventional `${service_home}/.bun/bin/bun` path (per `render_systemd_unit()` in `scripts/setup-multiuser-for-ubuntu.sh`), not merely the runtime that happened to launch the smoke script itself (no version drift), and — negative assertions — the MCP token / provider key appear in neither `/proc/<pid>/cmdline` nor `/proc/<pid>/environ`. A configured absolute `EMBEDDED_AGENT_BUN_PATH` that does not exist on disk is a probe-cannot-run condition (exit 2), not a failure. Exit codes 0/1/2 per `os-environment-coupling.md`; documented in the multi-user setup guide.
