...
**Eligibility gate (`deliverInitialPromptOnActivation`, in-memory only).** Only the session's *initial* embedded-agent worker — the one created together with the session, with a non-empty `initialPrompt` — is eligible. A worker added later via the generic add-worker route (`routes/workers.ts` → `sessionManager.createWorker(sessionId, body, continueConversation)`, no `initialPrompt` argument) is never eligible, even if the session happens to carry an `initialPrompt` from its own creation. `WorkerLifecycleManager.createWorker`'s `embedded-agent` branch sets `deliverInitialPromptOnActivation: !!initialPrompt?.trim()` on the `InternalEmbeddedAgentWorker` at creation time. This flag is **not persisted** (mirrors `subprocess`/`stdin` being null-on-restore, not part of `PersistedWorker`): a session whose initial embedded-agent worker was created but never activated before a server restart will not receive the delivery after restart — an accepted narrow edge case, same category as other v1 "restart" limitations, not a bug to fix here.
