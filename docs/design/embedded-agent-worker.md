...
4. **Rules** — every `<gitRoot>/.claude/rules/*.md`, sorted by file name. A rule's `paths:`/`globs:` frontmatter (either spelling; string or array; the multi-line YAML list this repo's own rules use) determines its scope: **unscoped rules** (no such frontmatter) are included eagerly, the same way an instruction segment is, under a `--- Rule: <origin> ---` delimiter. **Scoped rules** are NOT included at session start — Phase A only lists their name and globs in a one-line index ("Rules that apply when you touch matching paths: ..."), so the model knows they exist without paying their content cost; lazily activating them on a matching tool call is Phase B (tracked as a checklist in [#1343](https://github.com/ms2sato/agent-console/issues/1343), not implemented here). No git root, or no `.claude/rules` directory, produces an empty layer silently — both are routine.
