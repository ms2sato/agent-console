...
- **`restore-info` (Transcript Restore, #1123).** A new `WorkerServerMessage` variant, `{ type: 'restore-info'; epoch: number; messageCount: number; repairedToolCallIds: string[]; completed: boolean }`, added to `packages/shared/src/types/session.ts`'s `WorkerServerMessage` union (and `WORKER_SERVER_MESSAGE_TYPES`). Sent ONLY when an activation's restore succeeded (never on restore failure or first-ever activation). Dual delivery (fast-path push to currently-attached connections right after reconstitution, before spawn; bootstrap re-delivery to every new connection for the lifetime of the incarnation), PLUS a third push once the new incarnation's `ready` event is observed server-side, re-sending the same message with `completed` flipped to `true` -- see [Transcript Restore § UI](#transcript-restore) for the full mechanism and rationale (`completed`'s corrected role is described there, Issue #1205).
...
- A persistent, non-dismissable note in the view: conversation resets when the worker or server restarts (the v1 worker-type inconsistency called out in [Design Decisions](#design-decisions)). **Since Transcript Restore (#1123)** this note's exact wording/condition depends on the worker's [Engine](../glossary.md#engine-embedded-agent--openai-api--sdk): `openai-api` workers keep the "restored automatically" wording (accurate — their conversation IS reconstructed); a `claude-sdk`-engine worker that actually had a prior transcript restored gets a distinct, more specific notice instead — the SDK engine's accepted transcript-restore divergence made visible, shipped in SDK Engine Phase 3 (Issue #1335) — see [embedded-agent-sdk-engine.md §4.3](embedded-agent-sdk-engine.md#43-ui-the-restore-divergence-notice-sdk-engine-phase-3).
...
- **`claude-sdk`** — enqueues `/compact` as a user message onto the SDK's own input queue after the turn ends, riding the SDK's own admission of that command. **This injection produces no server-side `user-message` echo**: the server-authored echo exists so a real human/API-caller message appears in the persisted transcript, and a synthetic `/compact` is neither. A fake user row saying `/compact` would misattribute the action to the user. If the probe ([#1400](https://github.com/ms2sato/agent-console/issues/1400) P2) finds `/compact` does not exist on this SDK, the tool instead returns an explanatory result saying this engine supports automatic compaction only — the tool stays registered, because an accurate explanation is an honest affordance, and de-registering it would leave the model unable to say why it cannot comply. It never silently routes into `openai-api`'s distillation.
...
- **When it takes effect.** For `openai-api` the flag is read at each turn boundary, so a change applies immediately. For `claude-sdk` the flag becomes SDK settings, which are composed at activation; whether a mid-session change can be reflected without a restart depends on [#1400](https://github.com/ms2sato/agent-console/issues/1400) P1a (`applyFlagSettings`). **Live reflection ships only if P1a confirmed it takes effect**; otherwise the toggle applies at the next activation, and the UI says so.
...
3. **The eviction structural reservation** — the `spawnClaudeCodeProcess` override and the persisted SDK session id. Unrelated to handoff; [#1336](https://github.com/ms2sato/agent-console/issues/1336) and PS4 depend on it (`embedded-agent-sdk-engine.md` §6).
...
**Summary fidelity** ([#1350](https://github.com/ms2sato/agent-console/issues/1350)) stays open and is re-pointed: its subject was the handoff distillation's fidelity, and it is now read as **the compaction summary's** fidelity. The concern transfers intact — the question "how much did we lose, and can the user tell?" is identical whether the summary seeds a successor conversation or replaces the head of this one.
...
- **Restoring state.** Derived client-side, but from a purely server-authoritative signal, not a new state enum: `restoring` is true if and only if the most recently ACCEPTED `restore-info` message (per the `epoch` staleness guard above) carries `completed: false`. No client-side `ready`-observation bookkeeping is consulted at all -- the client does not need to reason about whether the current incarnation's `ready` has been folded into its local replay yet; the server already told it. While derived-true, the view shows a loading state: `Restoring conversation from N previous messages...`, using the received `messageCount`.
