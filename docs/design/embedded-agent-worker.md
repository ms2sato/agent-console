...
- `exited` events render as a plain historical transcript row (no affordance); a separate Restart action, driven by the worker's CURRENT exit state rather than by which row is last in the transcript, renders as a sibling element below the transcript while that state holds (re-activation = fresh conversation). See the `reason: 'evicted'` subsection below for the evicted-row's own no-affordance rendering, and R1 (#1455) for why the affordance moved out of the transcript.
