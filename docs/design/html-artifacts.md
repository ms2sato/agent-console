...
- **P6 — browsers automatically send the `Sec-Fetch-Dest` Fetch Metadata request header.** The raw serving endpoint (§4) uses it to distinguish a genuine iframe-embedded load (`Sec-Fetch-Dest: iframe`) from everything else — a top-level browser navigation, a plain `fetch()`/`curl` call with no Fetch Metadata headers at all — and redirects anything that is not exactly `iframe` to the viewer shell (§4) instead of serving raw bytes. **The header's ABSENCE also redirects; there is no fallthrough to serving bytes.** This is what makes P6 a UX premise, not a safety premise: an old browser or an unusual client that never sends the header simply sees the jailed shell instead of raw bytes it might have wanted directly at the top level — safety holds either way, because of this fail-closed default. Verified by the V1 probe (arm 7) and a route-level test.
...
| Artifact scripts run (`'unsafe-inline'`); an artifact can visually imitate anything, including a login form | §3.2 — cannot send what it collects: fetch/XHR blocked by CSP (`connect-src` fallback), form POST blocked by the sandbox's missing `allow-forms`, navigation blocked by the shell's `frame-src` plus the `Sec-Fetch-Dest` gate that makes the shell mandatory (§3.3 P6/P7); visual deception reduced to a social problem |
...
| Boundary rests on named premises P1–P7 (opaque-origin support, `SameSite=Lax` cookie, unfiltered embedded tools-list, `/api`-mount auth, trusted viewer population, `Sec-Fetch-Dest` fail-closed default, embedder-governs-child-navigation) | §3.3 — each with its probe/test, and each with the consequence of losing it stated |
