...
- **Memo (`packages/client/src/components/sessions/MemoPanel.tsx`)** -- **an active member of this class, mitigation not yet applied.** `MemoPanel.tsx` renders its content via `<Markdown remarkPlugins={[remarkGfm]}>{content}</Markdown>` (`react-markdown`), so a markdown link becomes a real anchor -- and `write_memo` is a registered MCP tool (`mcp-server.ts`), so an agent can write that content. Both halves of this section's own membership test (the last bullet below) are satisfied: an MCP caller can write the field, and the client turns the field's content into something a click follows. Tracked and being addressed under [Issue #1391](https://github.com/ms2sato/agent-console/issues/1391) -- do not treat memo as out of scope on the strength of an earlier draft of this document that said otherwise. `react-markdown` v10's default `urlTransform` drops dangerous schemes, so the residual gap there is narrower than bookmark's (no scheme-injection surface) -- what remains is host visibility and `rel` on the rendered anchor; see #1391 for the measured specifics.
...
`POST /api/bookmarks` continues to set `origin: 'user'` unconditionally. `GET /api/bookmarks` now returns `origin` in each row.

**"A human at a browser is the only REST caller" holds only under multi-user auth.** In multi-user mode, `authMiddleware` requires a valid signed-in cookie, so reaching this route at all implies a human session; `origin: 'user'` is a meaningful claim there. Under `AUTH_MODE=none` (single-user), `SingleUserMode.authenticate()` ignores whatever token is presented and always returns the cached server-process user -- so any local process that can reach the loopback port (including a `Bash` tool invocation by an agent working in this codebase) can call `POST /api/bookmarks` and produce an `origin: 'user'` row indistinguishable from a human's. This is inside the accepted single-user trust model (an agent with shell access can already do arbitrary things as that user), not a new hole -- but it means the `origin` badge is **advisory, not load-bearing, in single-user mode**: don't read "no badge" as proof a human clicked "Add bookmark." The click-time guarantee that does not depend on `origin` at all -- the host line (§3.3/§7) -- is what remains load-bearing regardless of auth mode.
...
2. **Host line** -- `new URL(bookmark.url).host`, rendered as an **independent text node outside the anchor**. This is the load-bearing structural choice: because it is a sibling element, not a child of the title's `truncate` container, no title length or CSS truncation can visually crowd it out. It is always rendered, including when `title` is null (title-null + host-line-present is not a special case -- the rule "host is always visible" is uniform regardless of whether a title exists). **The host line wraps (`break-all` / `overflow-wrap: anywhere`), never clips.** A host is attacker-choosable text too (up to a 253-character DNS name, 63-character labels) -- a fixed-width unbroken line would let a crafted long host (e.g. `github.com.<63-char label>.<63-char label>....attacker.example`) overflow the panel with only its harmless-looking prefix visible and the actual registrable suffix pushed off-screen. That is the same "pushed out of the click-time decision" failure P4 exists to prevent, reappearing as the host overflowing itself rather than being overflowed by the title. `truncate` / `text-overflow: ellipsis` on this line is therefore **forbidden**, not merely unnecessary -- clipping reproduces the exact hole wrapping closes.
...
**Realtime refresh (Issue #1520 / #1632).** Every bookmark create and delete -- through either transport -- emits a trigger-only `/ws/app` message (`bookmark-created` / `bookmark-deleted`, carrying at most `{ sessionId, bookmarkId }`, never content) after the repository write succeeds. `POST /api/bookmarks` and `DELETE /api/bookmarks/:id`, and the `create_bookmark` / `delete_bookmark` MCP tools, all call through the same shared writer (`packages/server/src/lib/artifact-bookmark-triggers.ts`), which is the single owner of the delete trigger's "owning session" resolution (the session whose panel query the bookmark is listed under, resolved from the deleted record's `sourceSessionId`; the MCP tools additionally supply their own calling session as a fallback when `sourceSessionId` is null, while the REST route supplies no fallback and skips the broadcast in that case). `useSessionBookmarks.ts` invalidates its scoped query key on receipt (N1: the broadcast is never itself a source of list content). Issue #1361's original invalidation-hint proposal is superseded by this mechanism -- there is no remaining 60s-staleTime gap for either transport.
...
- Do not restate the two-condition list-scoping rationale (`sourceSessionId` as secondary filter, not authorization) in new JSDoc at each call site -- point at [`docs/design/html-artifacts.md`](html-artifacts.md) §4.2, the canonical home (as of [PR #1395](https://github.com/ms2sato/agent-console/pull/1395)/[Issue #1385](https://github.com/ms2sato/agent-console/issues/1385); `bookmark-repository.ts`'s own JSDoc is now a pointer plus a one-line summary, not the canonical text itself).
...
- [Issue #1391](https://github.com/ms2sato/agent-console/issues/1391) -- the same-class re-derivation for `MemoPanel.tsx` (§3.4).
