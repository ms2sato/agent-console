...
**Realtime refresh (Issue #1520 / #1632).** Every bookmark create and delete -- through either transport -- emits a trigger-only `/ws/app` message (`bookmark-created` / `bookmark-deleted`, carrying at most `{ sessionId, bookmarkId }`, never content) after the repository write succeeds. `POST /api/bookmarks` and `DELETE /api/bookmarks/:id`, and the `create_bookmark` / `delete_bookmark` MCP tools, all call through the same shared writer (`packages/server/src/lib/artifact-bookmark-triggers.ts`), which is the single owner of the delete trigger's "owning session" resolution (the session whose panel query the bookmark is listed under, resolved from the deleted record's `sourceSessionId` -- never the deleting call's own identity). `useSessionBookmarks.ts` invalidates its scoped query key on receipt (N1: the broadcast is never itself a source of list content). Issue #1361's original invalidation-hint proposal is superseded by this mechanism -- there is no remaining 60s-staleTime gap for either transport.
