...
| Feature | Current location | Status | Notes |
|---|---|---|---|
| Keyboard input | `Terminal.tsx:636-638` | covered (with structural inventory gap — see note) | Hidden textarea; keydown → escape sequences. **Structural gap surfaced by Issue [#985](https://github.com/ms2sato/agent-console/issues/985) (dogfood 2026-07-06): the "covered" verdict was derived by grepping the legacy `Terminal.tsx`, which did NOT contain the keyboard-shortcut conversion table — xterm.js's `evaluateKeyboardEvent` absorbed browser `keydown` → PTY bytes silently. As a result, the inventory missed Shift+Tab, modifier-encoded arrows, Home / End / Delete / PageUp / PageDown, Alt+letter (readline Meta), F1-F10, Ctrl+Space, Ctrl+Shift+{2,6,-}, application-cursor-mode (DECCKM) form switching, and other shortcuts entirely. The full comparison against xterm.js `src/common/input/Keyboard.ts` (master @ `8aab310`) plus the Phase B fix scope live in [`docs/audits/terminal-key-handling-parity.md`](../audits/terminal-key-handling-parity.md). **Lesson for future promotion audits: parity inventories must reference the upstream library / dependency being retired, not only the local wrapper that consumed it.** |
