...
6. **State restoration relies on AgentConsole's existing server-side output persistence** (`packages/server/src/lib/worker-output-file.ts` file-backed output + the WebSocket `request-history` offset catch-up served via `worker-lifecycle-manager.ts:698` `readHistoryWithOffset`). No new client-side cache layer (IndexedDB, snapshots, serialized buffers) may be introduced in `labs/` or its successors. Exactly two restoration paths exist: **connection lost → server catch-up stream** (offset delta), and **tab/route switch → the long-lived instance** (invariant 1). (Owner direction, 2026-07-02.)
...
Scope (all inside `labs/terminal-poc/`). This PR is the implementation of **invariant 6**: every restoration path below leans on the server's `worker-output-file` persistence + offset catch-up, never on a client-side cache.
