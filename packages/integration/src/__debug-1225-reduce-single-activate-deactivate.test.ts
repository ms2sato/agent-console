...
describe('Issue #1225 reduce -- single activate+deactivate, NO message, NO wait', () => {
  ...
  it('creates ctx, real servers, agent, session, worker, activates once, deactivates immediately', async () => {
    ...
    realCwd = path.join(os.tmpdir(), `ac-1225-reduce-single-activate-deactivate-${crypto.randomUUID()}`);
