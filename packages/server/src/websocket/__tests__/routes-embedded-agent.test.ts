...
describe('Worker WebSocket: embedded-agent branch', () => {
  ...
  it('rejects the retired embedded-handoff message rather than acting on it', async () => {
    // #1401 removed `embedded-handoff` from the client-message union; the
    // WS layer's default branch is what an old client (or a stale tab) now
    // meets. It must be rejected explicitly -- silently ignoring it would
    // leave such a client waiting forever for a handoff that will never
    // happen.
    const { sessionId, workerId } = await createEmbeddedAgentSession();
    const { handlers, mockWs } = openConnection(sessionId, workerId);
    await waitFor(() => fake.captured.length === 1);
    const writesBefore = fake.stdinWrites.length;
    mockWs.sentMessages.length = 0;

    handlers.onMessage({ data: JSON.stringify({ type: 'embedded-handoff' }) }, mockWs);

    await waitFor(() => mockWs.sentMessages.length > 0);
    expect(JSON.parse(mockWs.sentMessages[0]).type).toBe('error');
    // And nothing reached the loop.
    expect(fake.stdinWrites.length).toBe(writesBefore);
    expect(mockWs.closeCalls.length).toBe(0);
  });
