...
describe('migration', () => {
  ...
  describe('schema migration v7: description column', () => {
    it('should add description column to repositories table when migrating from v6', async () => {
      ...
      expect(result.rows[0]?.user_version).toBe(45);
  ...
  describe('schema migration v14: created_by column', () => {
    ...
    it('should advance schema version past v14 to the latest', async () => {
      ...
      // confirms the v14 step is part of that chain (final version is 45).
      const db = await initializeDatabase(':memory:');
  ...
  describe('schema migration v16: timers table', () => {
    ...
    it('should advance schema version past v16 to the latest', async () => {
      // initializeDatabase runs every migration up to current; this just
      // confirms the v16 step is part of that chain (final version is 45).
      const db = await initializeDatabase(':memory:');
