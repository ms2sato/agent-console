...
sessions.delete('/:id/invalid', async (c) => {
...
sessions.post('/', validateBody(CreateSessionRequestSchema), async (c) => {
...
sessions.patch('/:id', validateBody(UpdateSessionRequestSchema), async (c) => {
  const sessionId = c.req.param('id');
...
sessions.get('/:sessionId/workers', async (c) => {
...
sessions.get('/:sessionId/branches', async (c) => {
  const sessionId = c.req.param('sessionId');
  const sessionManager = getSessionManager();
  const session = sessionManager.getSession(sessionId);
...
sessions.get('/:sessionId/commits', async (c) => {
  const sessionId = c.req.param('sessionId');
...
sessions.get('/:sessionId/pr-link', async (c) => {
  const sessionId = c.req.param('sessionId');
  const sessionManager = getSessionManager();
  const session = sessionManager.getSession(sessionId);
...
sessions.post('/:sessionId/workers', validateBody(CreateWorkerRequestSchema), async (c) => {
  const sessionId = c.req.param('sessionId');
...
// Delete a worker
sessions.delete('/:sessionId/workers/:workerId', async (c) => {
  const sessionId = c.req.param('sessionId');
...
sessions.post('/:sessionId/workers/:workerId/restart', validateBody(RestartWorkerRequestSchema), async (c) => {
  const sessionId = c.req.param('sessionId');
  const workerId = c.req.param('workerId');
...
sessions.get('/:sessionId/workers/:workerId/diff', async (c) => {
  const sessionId = c.req.param('sessionId');
  const workerId = c.req.param('workerId');
...
sessions.get('/:sessionId/workers/:workerId/diff/file', async (c) => {
  const sessionId = c.req.param('sessionId');
  const workerId = c.req.param('workerId');
